...
function OtherTools($strDatabase, $strPHP)
{
	...
	{
		$out.="<a href=\"tabletotablecopy.php" . $strDBPart . "\">Table to table copy.</a><br/>\n";
	}
	...
	{
		$out.="<a href=\"dbdiff.php" . $strDBPart . "\">Analyze database changes.</a><br/>\n";
	}
	if(file_exists("sqlimport.php"))
	{
		$out.="<a href=\"sqlimport.php" . $strDBPart . "\">Import a database from SQL.</a><br/>\n";
	}
	$out.="<a href=\"" . $strPHP .  $strDBPart . "&" . qpre . "mode=backup\">Backup database to a SQL file.</a><br/>\n";

	if(file_exists("sqlexport.php"))
	{
		$out.="<a href=\"sqlexport.php" . $strDBPart . "\">Comprehensive SQL export tools.</a><br/>\n";
	}
	if(file_exists("columnscanner.php"))
	{
		$out.="<a href=\"columnscanner.php" . $strDBPart . "\">Search the entire <strong>" . $strDatabase . "</strong> database.</a><br/>\n";
	}

	if(file_exists("dbmap.php"))
	{
		$out.="<a href=\"dbmap.php" . $strDBPart . "\">Show/create relationship maps of the <strong>" . $strDatabase . "</strong> database.</a><br/>\n";
	}
	...
	{
		$out.="<a href=\"tableconcordance.php" . $strDBPart . "\">Copy information between similar tables using concordances</a>.<br/>\n";
	}
	if(file_exists("fieldfinder.php"))
	{
		$out.="<a href=\"fieldfinder.php" . $strDBPart . "\">Search the entire <strong>" . $strDatabase . "</strong> database for tables having a given field name.</a><br/>\n";
	}
	if(file_exists("deleterange.php"))
	{
		$out.="<a href=\"deleterange.php" . $strDBPart . "\">Delete a range of IDs from a table, and also delete  all dependent columns in other tables.</a><br/>\n";
	}
...
//Breadcrumb nav for the admin tools
{
	...
	if(defined("databasepickerlink"))
	{
		...
		if($intArgs >1)
		{
			$out.="<a href=\"databases.php\">" . $strLinkName . "</a>";
		}
		else
		{
			$out.=  $strLinkName;
...
function GenericForm($arrFieldNames,  $arrLabels, $arrDefaults, $arrFieldConfigs, $strPHP, $strButtonLabel="Save", $width=630, $arrSizes="")
//Simplest-possible conversion of field names, labels, and defaults into an actual form.
{
	...
	for($i=0; $i<count($arrFieldNames); $i++)
	{
		...
		{
			//echo $thisfieldname . "+<BR>";
			if(array_key_exists($thisfieldname, $arrFieldConfigs))
			{
				$thisconfig=strtolower($arrFieldConfigs[$thisfieldname]);
				//echo $thisconfig . "<BR>";
			}
		}
		...
		if(contains($oldfieldname, "password")  && contains($arrFieldNames[$i], qpre . "password") )
		{
		...
		{
			$size=parseBetween($thisconfig, "size:", " ");
			//echo $thisconfig . "*" . $size . "<BR>";
		}
		if (contains($thisconfig,"label:'"))
		{
			$label=parseBetween($thisconfig, "label:'", "'");
			//echo $thisconfig . "*" . $size . "<BR>";
		}
		if (contains($thisconfig,"hid"))
		{
			$out.=HiddenInputs(Array($arrFieldNames[$i]=>$strThisDefault),"") . "\n";
		}
		else
		{
			if($size=="")
			...
			{
				$inputtag=GenericInput($arrFieldNames[$i], $strThisDefault,  false,  "",   "",  "", $type,$size, "", $height);
			}
			$out.=htmlrow($strThisBgClass, gracefuldecay($label, $arrLabels[$i], $arrFieldNames[$i]), $inputtag);
		}
		$strFieldsAdded.=" " . $arrFieldNames[$i];
		$oldfieldname=$arrFieldNames[$i];
	...
	$out.=htmlrow($strOtherLineClass, "&nbsp;", GenericInput(qpre . "save", $strButtonLabel));
	$out.= "</form>\n";
 
	$out= TableEncapsulate($out, false, $width);
	return $out;
}
...
//returns the content within the brackets, starting from both ends and moving inward
{
	...
	if(contains($strIn,  $chrStartBracket) && contains($strIn,  $strEndBracket))
	{
		...
		else //we parse the content from both ends, eliminating brackets in quoted regions
		{
			...
			//echo "$";
			$out=substr($strIn, $start, $end-$start);
			if($bwlRefHandback)
			{
...
//bwlHiddenReturn allows me to pass the actual string value of the dropdown in a specially-labeled hidden field.
{
	...
	if ($records)
	{
		...
		foreach($records as $k)
		{
			...
			{
			 	//echo truncate($strOldLabel, 35) . " ---- " . trim(truncate($strLabel1, 35))  . " --- " . (trim(truncate($strOldLabel, 35))==trim(truncate($strLabel1, 35))) .  "\n";
			 
				if(trim(truncate($strOldLabel, 35))==trim(truncate($strLabel1, 35)) )
				{
					...
					if($pretruncated!="")
					{
						$strLabel= truncate("..." . $pretruncated, 30);
					}
					else
					{
			...
			$arrOut[$outcount] = "<option value=\"". $k[$strIDField] . "\" " . $strSel .  ">". truncate($strLabel, 35) ."\n";
...
function TableExplanationToTableCreationSQL($strDatabase, $strTable, $bwlGuessPK=true, $bwlGuessAutoInc=true)
//take a table explanation and turn it into create table sql
{
	...
	foreach($records as $record)
	{
		...
		$extra=$record["Extra"];
		$thisLine="`" . $columnname . "` " . $type . " " . $strNullString . " " . $extra . ",\n";
		$strSQL.=$thisLine;
		$columns++;
	}
...
//explains the columns across several tables linked by relations in the relation table or do a simple explain
{
	...
	if( $db_type=="odbc")
	{
		return odbc_DBExplain($strDatabase, $strTable, $bwlFollowRelations);
	}
...
//Is this TF user a super admin?  This is the highest level of power in the system.
{
	...
	if(TableExists($strDatabase, $a["table"]))
	{
		...
		if (count($records)>0  && (authtype!="xcart"  || !defined("authtype")))  //if there are no superadmins then everyone is a superadmin!
		{
			...
			$strSQL="SELECT  is_superadmin from  " . $strDatabase . "." . $a["table"] . " WHERE " . $a["username"]. " = '" . singlequoteescape($strAdmin) . "'";
			//echo $strSQL;
			$records = $sql->query($strSQL);
			$record=$records[0];
...
//Judas Gutenberg Dec 1 2008
{
	$strClassFirst="bgclassfirst";
	$strClassSecond="bgclasssecond";
	$preout.= "<script src=\"tablesort_js.js\"><!-- --></script>";
...
function ScaleEmbeddedIntegers($in, $scale=61)
{
...
function EncryptQS($qs, $humanreadable="", $intHumanReadableSize=19, $intAbsurdityRate=0)
{
	$subencrypted=addletters(ScaleEmbeddedIntegers($qs, 701),encryptionkey, "numericnodelimiter", $intAbsurdityRate);
...
function DecryptStringToQS($qs="", $bwlJustReturnArray=false, $bwlDebug=false, $bwlRejectWild=true, $strResultArray="request")
{
	if($qs=="")
	{
		$qs=$_REQUEST[qpre . "eq"];
	...
	if(is_array($arrQS))
	{
		...
		//echo $partWeParse . "=2<br>";
		$partWeParse=ScaleEmbeddedIntegers($partWeParse, (1/701));
