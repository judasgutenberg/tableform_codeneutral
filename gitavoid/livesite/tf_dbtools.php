...
include_once('tf_functions_color_math.php');
...
function EnterSQL($strDatabase, $strPHP, $strSQL, $trunc, $bwlDirectediting, $strPostRunScript="", $language_id="")
{
	...
	$out.="<br/>{<a href='#' onclick='javascript:document.BForm." . qpre . "actscript.value=document.BForm." . qpre . "actscript.value.replace(\"{\", \"\").replace(\"}\", \"\")'>eliminate squiggly brackets from Drupal SQL</a>}";
	$out.="</td>\n";
	...
	$out.="<a href=\"" . ( stripslashes(AllRequestToURL())) . "\">URL of this page's state</a> ";
