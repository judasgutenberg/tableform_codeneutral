...
function RenderQuestions($strPHP, $strDatabase, $intQuestionnaire_id, $intUserID, $answer_group_id, $strMappingTableAvoidList="", $strQuestionAvoidList, $width="100%", $bwlSuppressSidebuttons=false)
{
//renders a questions for a questionnaire 
	...
	if(!$accept_multiple)
	{
		...
		$out.= "<table border=\"0\" cellspacing=\"1\" cellpadding=\"2\" class=\"" .$strLineClass  . "\" width=\"" . $width . "\">\n";
		
	 	$out.=htmlrow("text_12bl", "<div align=\"right\"><img src=\"" . imagepath . "/pointingfinger_left.gif\"></div>", "Don't forget to build your profile to its fullest<br> by filling in any relevant categories.");
	...
	{
		$out.= MultipleAnswerTable($strDatabase, $intQuestionnaire_id, $intUserID, $strMappingTableAvoidList, $bwlSuppressSidebuttons);
...
function SaveQuestionnaireAnswers($strDatabase, $intUserID, $strMappingTableAvoidList="")
{
	...
	if($fieldcount>0)
	{
		...
		if($intMaxGroupID>$fieldcount-1)
		{
			for($i=$fieldcount-1; $i<$intMaxGroupID; $i++)
			{
				...
				//echo "<br>" . $strSQL . "<br>";
				$records = $sql->query($strSQL);
				//echo qpre . "grid-" . $arrQuestionColumnMapFlipped["|" . $mappedtableinsertcolumn ]. "-" .  $i ;
	...
	if( trim($mappingTableIDs)!="")
	{
		//delete mapping table relations no longer being used!!
		$strSQL="DELETE FROM " . $strDatabase . "." .  $usermappingtable . " WHERE NOT " .  IDListSQLForWhere($mappingTableIDs, $mappedtableinsertcolumn, "OR") . " AND user_id="  . 	$intUserID;
		$records = $sql->query($strSQL);
		//echo $strSQL;
	}
	//echo "_____" .  $mappingTableIDs;
	//echo "<p><a href=\"javascript:domdumpwindow()\">dump</a>";
}
...
function MultipleAnswerTable($strDatabase, $intThisQuestionnaireID, $intUserID, $strMappingTableAvoidList="", $bwlSuppressSidebuttons=false)
{
	$sql=conDB();
	$out="";
	$dataHTML="";
	...
	//$strRegularAddLink=$addlink. HiddenInputs(array("group_id[]"=>$group_id));
	//$arrParam[count($arrParam)]=$strRegularAddLink;
	...
	$out.="<tr><td align=\"left\" colspan=\"" . intval((count($arrLabels)-2)-$intHiddens) . "\">" . MASelectedTools($bwlSuppressSidebuttons). "</td><td align=\"right\" >" .  GenericInput(qpre . "submit", "Save") . "</td></tr>\n";
	$out.="</table>\n\n";
	...
	$out.="<div class=\"text_11b2\">Click the <a href=\"javascript:addQrowAtEnd('')\">";
	if(bwlSuppressSidebuttons)
	{
		$out.="<img border=\"0\" src=\"" . imagepath . "/new.gif\">";
	}
	else
	{
		$out.="<img border=\"0\" src=\"" . imagepath . "/insert_" .$listaddside . ".gif\">";
	}
	$out.="</a> button to add a new <strong>" . $strSingularizedName  . "</strong>.  Click an existing <strong>" . $strSingularizedName . "</strong> to select it.  Then click the  arrows to move it up or down or the <a href=\"javascript:deleteQrowSelectedOrAtEnd('')\"><img border=\"0\" src=\"" . imagepath . "/delete.gif\"></a> button to delete it.</div><p/>";

	//echo "<a href=\"javascript:domdumpwindow()\">dump</a>";
	return $out;
}

function answerformwrap($answer, $x, $y, $sql_data_type, $foreigntable, $bwlHidden=false, $width=20, $height=1, $intUserID="", $bwlNoForm=false,  $strMappingTableAvoidList="", $arrLimiting="", $strPreferredNameField="")
{	
	...
	if($limitingvalue=="")
	{
		$limitingvalue=$intUserID;
	}
	if($arrLimiting==""  && $intUserID!="")
	{
		$arrLimiting=Array("user_id"=>$intUserID);
	...
	{
		//echo "&";
		$out.="" . GenericInput(qpre . "grid-" . $x . "-" . $y, $answer, false, "", $strStyle, $strClass, "hidden" , 1, $js, 1);
	}
	else 
	{
		if($bwlNoForm   )
		...
		else
		{
			...
			//foreigntablepulldown($strDatabase, $strTable, $strIDField, $intDefault, $strLabelField="", &$namereturn, $bwlHiddenReturn=false, $strPreferredNameField="")
				...
				//echo $foreigntable . " " . $idfield . " " . $answer . "<BR>";
				//Narrowedforeigntablepulldown($strDatabase, $strTable, $strIDField, $intDefault, $strLabelField="", &$namereturn, $bwlAutoGo=false, $strPreferredNameField="", $styleclass="",  $strNarrowingIDName="", $intNarrowingID="", $strAvoidTableList="", $arrLimiting="")
				$pulldown=Narrowedforeigntablepulldown($strDatabase, $foreigntable, $idfield, $answer,  qpre . "grid-". $x . "-" . $y, $namereturn, "",$strPreferredNameField, $selectStyle, "" , "", $strMappingTableAvoidList, $arrLimiting);
				//function foreigntablepulldown($strDatabase, $strTable, $strIDField, $intDefault, $strLabelField="", &$namereturn, $bwlHiddenReturn=false, $strPreferredNameField="", $styleclass="", $strWhereClause="")
				$wherclause=ArrayToWhereClause($arrLimiting);
				//$pulldown=foreigntablepulldown($strDatabase, $foreigntable, $idfield, $answer,  qpre . "grid-". $x . "-" . $y, $namereturn, "",$strPreferredNameField, $selectStyle, $wherclause);

				//code to change recurrence from "none" to "once"
				if($foreigntable=="recurrence")
				{
					$pulldown=str_replace( ">none", " >once", $pulldown);
				}
				$out.=$pulldown;
			}
		}
...
function MASelectedTools($bwlNewButton=false, $bwlNoUpDown=false, $imageextension="gif", $extratags="")
{
	$out="";
	//$out.="move selected: ";
	if(!$bwlNoUpDown)
	{
		$out.="<a href=\"javascript:MATRaction('up','')\"><img  " . $extratags. " onmouseover=\"glow(this, 'on')\"  onmouseout=\"glow(this, 'off')\" border=\"0\" src=\"" . imagepath . "/uparrow." .  $imageextension . "\"></a>";
		$out.="   ";
		$out.="<a href=\"javascript:MATRaction('down','')\"><img " . $extratags. " onmouseover=\"glow(this, 'on')\"  onmouseout=\"glow(this, 'off')\" border=\"0\" src=\"" . imagepath . "/downarrow." .  $imageextension . "\"></a>";
	}
	$out.="   ";
	$out.="<a href=\"javascript:MATRaction('delete','')\"><img " . $extratags. " onmouseover=\"glow(this, 'on')\"  onmouseout=\"glow(this, 'off')\" border=\"0\" src=\"" . imagepath . "/delete." .  $imageextension . "\"></a>";
	if($bwlNewButton)
	{
		$out.="   ";
		$out.="<a href=\"javascript:addQrowAtEnd('')\"><img " . $extratags. " onmouseover=\"glow(this, 'on')\"  onmouseout=\"glow(this, 'off')\" border=\"0\" src=\"" . imagepath . "/new." .  $imageextension . "\"></a>";
	}
 	return $out;
}
...
//obsolete:  this function is probably going away, but for now i need it because of that join it knows how to do
function Narrowedforeigntablepulldown($strDatabase, $strTable, $strIDField, $intDefault, $strLabelField="", &$namereturn, $bwlAutoGo=false, $strPreferredNameField="", $styleclass="",  $strNarrowingIDName="", $intNarrowingID="", $strAvoidTableList="", $arrLimiting="", $strWhereClause="")
	{
		//echo var_dump($arrLimiting) . "<P>";
		//echo  $strTable . "<br>";
		...
		{
			if(contains( $strPreferredNameField, " "))
			{
				$arrPreferred=explode(" ", $strPreferredNameField);
				$strNameField = $arrPreferred[0];
				$strNameField2 = $arrPreferred[1];
			}
			else
			{
		...
		if ($strNarrowingIDName!="")
		{
			...
			else if (FieldExists($strDatabase, $strTable, $strNarrowingIDName))
			{
				//echo "|<br>";
		...
		else if(is_array($arrLimiting))
		{
			//echo "$";
			$strLimitingSQL="";
			
			foreach($arrLimiting as $k=>$v)
			{
				if (FieldExists($strDatabase, $strTable, $k))
				{
				//echo "$";
					$strLimitingSQL.=" AND  " . $k . "='" .$v . "' ";
				}
				else
				{
					 $strSQL=LookupAPotentialLimitingJoin($strDatabase, $strTable, $k, $strAvoidTableList);
					 $strLimitingSQL.=" AND  " . $k . "='" .$v . "' ";
				}
			
			}
			$strSQL .=" WHERE " . substr($strLimitingSQL, 4);
		...
		if ($records)
		{
			...
			foreach($records as $k)
			{
				...
				// }
				 ...
				 $strSel="";
				 $strLabel=$strLabel . " " . $strLabel2;
				// echo $k[$strIDField] . "-" . $intDefault . "<BR>";
...
function InsertAtArrayPosOne($arrIn, $strItemIn)
{
	$strStore=array_shift($arrIn);
	array_unshift($arrIn,$strItemIn);
	array_unshift($arrIn,$strStore);
	return($arrIn);


}

function IDListSQLForWhere($strIDList, $fieldName, $type="OR")
{
	$arrList=explode(" ", trim($strIDList));
	$out="";
	$outcount=1;
