...
$strPK="login_id";
$errors="";
$bwlSavePromoCode=false;
$bwlNewOffice=false;
$bwlDoNotPermitOfficeReg=true;
...
if($promo_code!="")
{
	$pcrec=GenericDBLookup(our_db, "promo_code", "code",  $promo_code);
	$pc_office_id=$pcrec["office_id"];
	if($pc_office_id>0)
	{
		$errors.="The promo code '" . $promo_code . "' you have chosen has already been used. <br/><a href='promo.php'>Enter another</a>.";
		$bwlDoNotPermitOfficeReg=true;
	}
	else if ($pcrec["promo_code_id"]!="")
	{
		$bwlSavePromoCode=true;
		$bwlDoNotPermitOfficeReg=false;
	}
	else
	{
		$errors.="The promo code '" . $promo_code . "' is not valid. <br/><a href='promo.php'>Enter another</a>.";
		$bwlDoNotPermitOfficeReg=true;
	}
}
		//echo $errors . "-" . $_POST[qpre . "save"] . "*";
if($_POST[qpre . "save"]!=""  && $errors=="")
{
	//echo "EEE";
//GenericFormSaver($strDatabase, $strTable, $strPrimaryKey, $strPKVal, $strFieldIgnoreList, $bwlDebug=false, $bwlWorkFromSchema=false, $bwlDontOverwriteWithNulls=true, $bwlCreateSchema=false)
	...
	$_POST["email"]=FilterString(strtolower($_POST["email"]), $strLimitspec, "");
	...
	if($bwlDoNotPermitOfficeReg && $bwlNewOffice )
	{
		$errors.="While the site is in beta you must use a valid promo code. <br/><a href='promo.php'>Enter one</a>";
	}
	if($errors=="")
	{
		...
		if($mode=="office"  && $bwlSavePromoCode)
		{
			UpdateOrInsert(our_db, "promo_code", Array("code"=>$promo_code), Array("office_id"=>$intPossibleModeID));
		}
		...
		if($bwlNewOffice )
		{
			...
			{	
				...
				header("location: index.php?" . qpre . "message=" . urlencode("Thank you for signing up your " . locationtype . "."));
			}
		}
...
?>
    ...
    <div class='righttab' style="background-image:url('images/<?=$imagetopurl?>');width: 568px;height:36px"><?=$tabtext?></div>
     <div style="background-image:url('images/pinkybg.png');width: 568px;background-repeat:repeat-y;"></div>
			...
			$arrFieldConfig=Array("office_name"=>"size:55", "subdomain"=>"size:55 info:'Your website address will be <em>your_choice</em>." . strtolower(sitename) . ".  Do not use spaces or punctuation in your subdomain.'", 
		"password"=>"size:25", 
		qpre . "password"=>"size:25 info:'Capitalization matters in passswords so type carefully.", "address"=>"size:55", "city"=>"size:55",  "zip"=>"size:55", "phone"=>"size:33",
		"email"=>"size:55", 
		...
		"first_phonecall"=>"class:!bgclassclear action:startover", "second_phonecall"=>"class:!bgclasscleardarker", "third_phonecall"=>"class:!bgclasscleardarkest",  "first_email"=>"class:!bgclassclear action:startover", "second_email"=>"class:!bgclasscleardarker", "third_email"=>"class:!bgclasscleardarkest", "phonecall_time"=>"action:startover");
			...
			if( $mode=="office")
			{
				...
				$arrDefaults[qpre . "pc"]=$promo_code;
