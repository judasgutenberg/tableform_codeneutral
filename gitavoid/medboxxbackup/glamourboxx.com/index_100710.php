...
if($officetoken!=""  || strtolower($officetoken)=="www" && $mode!= "office")
{
...
<br>
<a href="reg.php"><div class="patientbutton"><div style="float:left; margin-top:12px;margin-left:-2px;margin-right:5px"><img  border="0" src="images/patrientreg_icon.png"></div> <br>Initial <?=customertype?> Registration.</p>  </div></a>
<br>
...
$hoursopen=gracefuldecay($ouruserrecord["hours_open"], "0*24");
$arrHoursopen=explode("*", $hoursopen);
$intHourStart=$arrHoursopen[0];
$intHourEnd=$arrHoursopen[1];
$calculatedverticalgrains=2+(($intHourEnd-$intHourStart)*60)/$unitsize;
$calculatedcalheight=15 *$calculatedverticalgrains-16;
 
?>
 

<form name="BForm">
<input type="hidden" name="hoursopen" value="<?=$hoursopen?>">
...
<input type="hidden" name="hourstart" value="<?=$intHourStart?>">
<input type="hidden" name="hourend" value="<?=$intHourEnd?>">
...
?><div style="position:absolute;top:<?=$calypos+43?>px;left:<?=$calxpos?>px;width:<?=$calwidth?>px;" class="weekcalendarsurround" id='idweekcalendar'></div><br>
	...
	<div style="width:1px;height:<?=$calculatedcalheight?>px;"></div>
...
<?
	...
	<br/>
	<div id="idblanktablabel" style="background-image:url('images/blankredtabtop.png');width:595px;height:28px; font-family:helvetica, arial;color:#<?=textcolorontheme?>;font-weight:bold;font-size:16px; text-indent:10px;line-height:180%">&nbsp;&nbsp;&nbsp;<span style="font-size:23px">Alerts</font></div>
	<div style="background-image:url('images/596_back.png');width:595px;height:200px; background-repeat:repeat-y;">
	<center>
...
//echo "vgrains:" . strval($calculatedverticalgrains ). " hours:" .  strval($intHourEnd-$intHourStart) . " calcheight:" .  $calculatedcalheight;
?>
