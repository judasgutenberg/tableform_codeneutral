...
function GenericFormSaver($strDatabase, $strTable, $strPrimaryKey, $strPKVal, $strFieldIgnoreList, $bwlDebug=false, $bwlWorkFromSchema=false, $bwlDontOverwriteWithNulls=true, $bwlCreateSchema=false, $strImageUploadKeyFieldName="")
	...
	foreach($placetogetdata as $k=>$v)
	{
		...
		if($k!="MAX_FILE_SIZE")
		{
			...
			//	for($i=0; $i<count($arrExtension); $i++)
				...
				$uploadpath= $imageroot . "/" . $placetogetdata[gracefuldecay($strImageUploadKeyFieldName, $strPrimaryKey)] . "." . $thisextension;
