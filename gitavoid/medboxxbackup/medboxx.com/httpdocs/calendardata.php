...
$strDatabase=our_db;
 ...
 ?>
 <script>
 //alert('dddd<?=$ouruserid?>');
 //alert('<?=$mode?>');
 </script>
 
 <?
 
if (($ouruserid<0 || $ouruserid=="" )&& 1==2 ) //we arent registered - but we allow you in if you're not
{
	...
	else //we are registered
{
	if($datecode!="")
	{
		//echo RenderCalendar($datecode,  $ouruserid, $strPHP);
		...
		if($mode=="office")
		{
			echo "calendardata='" . CalendarData($datecode,  $ouruserid, "", $strPHP, $mode)."';\n";
		}
		else
		{
			echo "calendardata='" . CalendarData($datecode,  "",$ouruserid, $strPHP, $mode)."';\n";
