...
if ( $_GET['action'] == 'finish' )
{
	...
	header('Location: edit.php?page=table_prop&db='.$_POST['db'].'&table='.$_POST['table']);
	exit;
}
			...
			<table width="100%" height="100%" cellspacing="0" cellpadding="3" border="0">
				<tr>
					<td width="150" valign="top" bgcolor="E5E5E5">
						...
						<form method="post" name="form1" action="edit.php?page=create_table&action=finish" onsubmit="checkCreateTableForm(<?php echo $_GET['columns']; ?>); return false;">
					...
					<td valign="top">
							...
							<tr>
								<td align=center bgcolor=e1e1e1>
									<b><a href="edit.php?page=view_databases&db=<?php echo $_GET['db']; ?>&action=create_database">Create DB</a> &nbsp;&nbsp;&nbsp;&nbsp;
									   ...
									   <a href="edit.php?page=<?php echo $sql->isLocked($_GET['db']) ? 'lock' : 'unlock';?>db&db=<?php echo $_GET['db']; ?>&go=edit.php?page=view_database%26amp;db={$_GET['db']}"><?php echo $sql->isLocked($_GET['db']) ? 'Unlock' : 'Lock'; ?> DB</a>
									</b>
								</td>
							</tr>
							<tr>
						...
						<table width="100%" cellspacing="1" cellpadding="3">
							...
							for ( $i = 0; $i < $_GET['columns']; $i++ )
							{
								...
								echo "\t\t\t\t\t\t\t\t<td><input type=\"text\" size=\"15\" name=\"cols[$i][name]\" tabedit=\"".( $i + 1 )."\"></td>\n";
