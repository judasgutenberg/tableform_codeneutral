...
if ( $_GET['action'] == 'rename_table' )
{
	...
	header('Location: edit.php?page=table_prop&db='.$_GET['db'].'&table='.$_GET['newtablename']);
	exit;
}
						...
						<table width="100%" cellspacing="1" cellpadding="3">
							...
							foreach ( $columns as $key => $value )
							{
								if ( $key != 'primary' )
								{
									...
									echo "\t\t\t\t\t\t\t\t<td><a href=\"edit.php?page=table_prop&action=edit_column&db={$_GET['db']}&table={$_GET['table']}&column={$name}\"><img src=\"images/small_edit.png\" border=\"0\" alt=\"Edit Field, {$name}\"></a></td>\n";
									...
									{
										echo "<a href=\"edit.php?page=table_prop&action=".( $columns['primary'] == $name ? 'drop' : 'add' )."key&db={$_GET['db']}&table={$_GET['table']}&column={$name}\"><img src=\"images/small_primary.png\" border=\"0\" alt=\"".( $columns['primary'] == $name ? 'Remove' : 'Set as' )." primary key, {$name}\"></a>";
									}
									else
									{
