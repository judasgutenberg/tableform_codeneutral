...
<form method="get" name="form" action="edit.php" onsubmit="rename_db(this.newdatabase.value); return false;">
			...
			<table width="100%" height="100%" cellspacing="0" cellpadding="3" border="0">
				<tr>
					<td width="150" valign="top" bgcolor="E5E5E5">
						...
						<form name="form1" method="post" action="edit.php?page=table_edit&db=<?php echo $_GET['db']; ?>&table=<?php echo $_GET['table']; ?>&action=edit&s=<?php echo $_GET['s']; ?>">
					...
					<td valign="top">
						...
						<table width="100%" cellpadding="3" cellspacing="1">
							...
							<tr>
								<td align=center bgcolor=e1e1e1>
									<b><a href="edit.php?page=table_prop&db=<?php echo $_GET['db']; ?>&table=<?php echo $_GET['table']; ?>">Structure</a> &nbsp;&nbsp;&nbsp;&nbsp;
									   <a href="edit.php?page=table_edit&action=browse_table&db=<?php echo $_GET['db']; ?>&table=<?php echo $_GET['table']; ?>">Browse</a> &nbsp;&nbsp;&nbsp;&nbsp;
									   <a href="edit.php?page=search&db=<?php echo $_GET['db']; ?>&table=<?php echo $_GET['table']; ?>">Search</a> &nbsp;&nbsp;&nbsp;&nbsp;
									   <a href="edit.php?page=table_edit&action=insert_row&db=<?php echo $_GET['db']; ?>&table=<?php echo $_GET['table']; ?>">Insert</a> &nbsp;&nbsp;&nbsp;&nbsp;
									   <a href="edit.php?page=export_database&db=<?php echo $_GET['db']; ?>&table=<?php echo $_GET['table']; ?>&export=-1">Export</a> &nbsp;&nbsp;&nbsp;&nbsp;
						...
						<table width="98%" cellspacing="1" cellpadding="3" align="center">
							...
							echo "\t\t\t\t\t\t\t\t\t\t\t\t<input type=\"button\" onclick=\"document.location.href='edit.php?page=table_edit&action=browse_table&db={$_GET['db']}&table={$_GET['table']}&s={$_GET['s']}&sdir=' + document.form1.elements['sortdir'].value + '&scolumn=' + document.form1.elements['sortcolumn'].value;\" value=\"Sort\"><br>\n";
							...
							echo "{$indent}\t\t\t\t</td>\n";
							echo "{$indent}\t\t\t\t<td>\n";
							echo "{$indent}\t\t\t\t\tGo to page: <select name=\"s\" onchange=\"document.location.href='edit.php?page=table_edit&action=browse_table&db={$_GET['db']}&table={$_GET['table']}&scolumn={$_GET['scolumn']}&sdir={$_GET['sdir']}&s=' + ( this.value );\" class=\"sel\">\n";
							...
							if ( !empty($rows) )
							{
								...
								foreach ( $cols as $key => $value )
								{
									if ( $key != 'primary' )
									{
										...
										echo "\t\t\t\t\t\t\t\t<td><b>";
										echo "<a href=\"edit.php?page=table_edit&db={$_GET['db']}&table={$_GET['table']}&s={$_GET['s']}&scolumn={$key}&sdir=$direction\" title=\"Sort Column; $direction\">";
								...
								if ( $_GET['s'] > 0 )
								{
									...
									echo "\t\t\t\t\t\t";
									echo "<input type=\"button\" value=\"Last Page\" onclick=\"document.location.href='edit.php?page=table_edit&action=browse_table&db={$_GET['db']}&table={$_GET['table']}&sdir={$_GET['sdir']}&scolumn={$_GET['scolumn']}&s={$last}'\">\n";
								}
								...
								if ( $_GET['s'] + 25 < $sql->table_count($_GET['table'], $_GET['db']) )
								{
									$next = $_GET['s'] + 26;
									echo "\t\t\t\t\t\t";
									echo "<input type=\"button\" value=\"Next Page\" onclick=\"document.location.href='edit.php?page=table_edit&action=browse_table&db={$_GET['db']}&table={$_GET['table']}&sdir={$_GET['sdir']}&scolumn={$_GET['scolumn']}&s={$next}'\">\n";
								}
