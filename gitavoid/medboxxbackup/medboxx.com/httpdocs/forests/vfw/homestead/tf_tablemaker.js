...
//decide when to provide an autoincrement checkbox 
	...
	//hardcoded!!!
	var intIndex=4;
	if(!thistypeselect[thistypeselect.selectedIndex])
	{
	 
		thistypeselect.selectedIndex=intIndex;
	}
			...
			if (thesechildren[j].nodeName.toLowerCase()=="td")
			{
				...
				for(k=0; k<these2children.length; k++)
				{
					if(these2children[k].nodeName.toLowerCase()=="a")
					{
						...
						if((these2children[k].id.indexOf("idrellink-")>-1))//turn off relation links for newly-added columns, may 31 2010
						{
							these2children[k].innerHTML='';
							these2children[k].href='';
						}
					}
