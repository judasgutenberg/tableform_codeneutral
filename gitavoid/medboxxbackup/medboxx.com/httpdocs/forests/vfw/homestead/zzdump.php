		...
		if ($intAdminType>1)
			{
				if ($submit!="")
				{
					...
					downloadData ($data, "text/plain", "Data Export for : " .  $strDatabase . " : " .  $strTable . ".txt");
				}
				else
				{
					$out.= DumpForm($strDatabase, $strTable);
					$out =  PageHeader($strTable . " Exporter", $strConfigBehave) . $out . PageFooter();
					return $out;
				}
			}
	}
...
function DumpForm($strDatabase, $strTable)
{
		...
		$out.= adminbreadcrumb(false, $strDatabase, $strPHP . "?" . qpre . "db=" . $strDatabase,  $strTable, qbuild($strPHP, $strDatabase, $strTable, "view", "", ""), "data exporter", "");
		$out.="<form name=\"BForm\" action=\"dump.php\" method=\"post\">\n";
