			...
			if ($intAdminType>1)
				{
					...
					$out.= ForeignKeyReferralLists($strDatabase, $strTable,$idfield , $id, "tableform.php", $strTheirTable, $rec, 200, $strDisplayMode, $strForeignLimit);
				}
		}
		//($strTitle, $strConfigBehave,$strForBackField="", $bwlIsStandalone=true, $bwlSuppressExternalHeader=false)
		$out =  PageHeader($strDatabase . " : Foreign Referrals", $strConfigBehave, "", true, true) . $out . PageFooter(true, false, true);
...
//inside a particular database, find all the tables that have a foreign key relationship with the given item and list them in grouped clusters, complete with various editing options
	...
	if ($strOurID!="")
	{
					...
					foreach ($records as $record )
					{
						...
						if ($strNameToUse=="")//in which i handle the case of complex mapping tables
						{
							...
							for ($intFKCounter=0; $intFKCounter<$intFKcount+1; $intFKCounter++)
							{
								...
								$strPartOfNameToUse= LookupName($strDatabase,$arrFKInfo[$intFKCounter][1],$arrFKInfo[$intFKCounter][2], $record[$arrFKInfo[$intFKCounter][0]], $strNameFieldFromRelationTable);
		...
		if ($out!="")
		{
			...
			$fkbase = qbuild("foreign.php", $strDatabase, $strOurTable, "", $strIDField, $strOurID) . "&" . qpre . "behave=noextras&" . qpre . "iddefault=" . $strOurID;
