...
function  GenericRSDisplay($strDatabase, $strPHP,$strLabel, $strSQL, $truncate, $intWidth, $strLinkFieldName="", $strLinkIDName="", $strAdditionalLink="", $strSuppressFields="", $bwlSuppressHeader=false, $bwlPrettyUpFieldNames=true, $intFieldLimit=10, $idencryptionstring="", $arrPostProcessing="")
{
	...
	if ($strSQL!="")
	{
		...
		if( count($rows)>0)
		{
			if ($strErrors=="" )
			{
				$out.= "<table id='idsorttable' width=\"" . $intWidth . "\" border=\"0\" cellspacing=\"1\" cellpadding=\"2\" class=\"" .$strLineClass  . "\"  >\n";
				$out.="<span class=\"heading\">" . $strLabel . "</span>";
				$out.= "</td></tr>\n";
							...
							foreach($record as $k=>$v)
							{
								if($fieldcount<$intFieldLimit)
								{
									if(!inList($strSuppressFields, $k) && !$bwlSuppressHeader)
									{
										...
										$out.="<a href=\"javascript: SortTable('idsorttable', " . $fieldcount . ")\">" . $kdisplay . "</a>";
										$out.="</td>";
					...
					foreach($record as $k=>$v)
					{
						...
						if($fieldcount<$intFieldLimit)
						{
							if(!inList($strSuppressFields, $k))
							{
								...
								if ($fieldcount==0  && $strLinkFieldName==""  || $strLinkFieldName==$arrFN[$fieldcount])
								{
								 
									$body="<a href=\"" .$strPHP . $strQSGlue . $strLinkIDName . "=" . $idtouse . "\">" . $body . "</a>";
...
function FrontEndTableForm($strDatabase, $strSQL, $strTable, $strIDField, $strPKValue, $strPHP,  $strHiddenList, $strDisplayOnlyList, $arrPassedInDefaults, $strConfigBehave, $strUser="", $bwlSuppressNewButton=false, $breadcrumb="", $bwlPlaintextPWD=false, $bwlSuppressSubmitButtons=false)
{
// echo $strPKValue . "=<br>";
	...
	if (count($descr)>0)
	{
		...
		foreach ($descr as $fieldlabel=>$info)
			{
				...
				if(!inList($noshowfields, $name))
				{
					...
					$intLen=intval(TypeParse($strTypeFull, 1));
					$width=$intLen;
					$bwlInvisible=false;
					$strPseudo="";
					...
					if($bwlDefaults)
					{
						$strDefault=$record[$fieldlabel];
					}
			 
					//echo "<b>" . $name . "</b> " . $strDefault . "<br>";
					$strDefault=gracefuldecay($arrPassedInDefaults[$fieldlabel],$strDefault) ;
							...
							if (count($arrFK)>0)
							{
								...
								$bwlHypLabel=true;
									...
									if ($count<101)
									{
										...
										$fieldform=foreigntablepulldown($strDatabase,$arrFK[0], $arrFK[1], $strDefault, $name, $bleh, false);
					...
					elseif  ($strType=="text" || $intLen>100)
					{
						...
						if (textarea=="textarea" || $strType!="text"  || !class_exists("wysiwygPro"))
						{
							...
							$fieldform="<textarea name=\"" .  $name . "\" cols=\"" . $width . "\" rows=\"" .$height . "\">" .  $strDefault . "</textarea>\n";
						}
						else
						{
							//using wysiwygPro
	   ...
	   onmouseover=\"this.className='btn btnhov'\" onmouseout=\"this.className='btn'\" name=\"" . qpre . "submit_clearid\" type=\"submit\" value=\"" . $strOtherButtonText ."\">\n";
		...
		if(1==1)
		{
			...
			$out.="<input  style=\"" . $strThisInputStyle . "\" class=\"btn\"
...
function RegularizeExplainedTable($strDatabase, $strTable)
{
	...
	$arrOut=Array();
	$ro=Array();
	$rs=DBExplain($strDatabase, $strTable);
	foreach($rs as $r)
	{
		//echo $fieldname . "-<br>";
		$fieldname=$r["Field"];
...
function GenericFormSaver($strDatabase, $strTable, $strPrimaryKey, $strPKVal, $strFieldIgnoreList, $bwlDebug=false, $bwlWorkFromSchema=false, $bwlDontOverwriteWithNulls=true)
...
//judas gutenberg
{
	...
	foreach($placetogetdata as $k=>$v)
	{
		...
		if($k!="MAX_FILE_SIZE")
		{
			...
			if (!beginswith($k, qpre))
			{
				...
				{
					if($bwlWorkFromSchema && GetFieldType($strDatabase, $strTable, $k)== "date")
					{
						//echo $k . " " . $v . "<br>";
						if (GetFieldType($strDatabase, $strTable, $k)== "date")
						{
	...
	$thisPK=UpdateOrInsert($strDatabase, $strTable, $arrID, $arrIncidental,true, $bwlDebug);
	return $thisPK;
}
