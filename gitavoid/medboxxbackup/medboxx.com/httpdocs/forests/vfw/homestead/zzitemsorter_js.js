...
function editItemG(id, idfield, table)
{
	...
	url="tableform.php?" + qpre + "table=" + table + "&" + qpre + "db=" +db + "&" + qpre + "mode=edit" + "&" + qpre + "behave=closeclickrecycle&" + qpre + "idfield=" + idfield + "&" + idfield + "=" + id + "&id=" + id;
...
function createItemG(thisid, idfield, table, joinfield, joinvalue, sortfield, filterfield, filterfieldvalue)
{
	...
	for(var j = 0; j < attrMax; j++)
   {
	...
	var db=document.BForm[qpre + "db"].value;
	url="tableform.php?" + qpre   + "noshow=" + sortfield + "&" + qpre + "table=" + table + "&" + qpre + "db=" +db + "&" + qpre + "mode=new" + "&" + qpre + "behave=closeclickrecycleupdateopener" + extrapair;
	//alert(db);
	height=300;
	width=500;
