...
function main()
{
	...
	$out =  PageHeader($strDatabase . " : table concordance", $strConfigBehave) . $out . PageFooter();
...
//Scan through two tables and try to find common items
{
	...
	$out="";
	$out.= adminbreadcrumb(false, $strDatabase, "tableform.php" . "?" . qpre . "db=" . $strDatabase,  "table concordance", "", "choose tables and fields", "");
	$tableheader="<table id=\"idsorttable\"  class=\"bgclassline\" cellpadding=\"0\" cellspacing=\"1\" border=\"0\" width=\"600\">\n";
	$out.= "<form method=\"post\" name=\"BForm\" action=\"tableconcordance.php\">\n";
	$out.= $tableheader;
...
function PreviousConcordances($strDatabase)
{
	...
	$tableheader="<table id=\"idsorttable2\"  class=\"bgclassline\" cellpadding=\"0\" cellspacing=\"1\" border=\"0\" width=\"650\">\n";
	$out.=$tableheader;
	$out.=htmlrow("bglineclass", 
	...
	foreach($records as $record)
	{
		...
		$fieldtowrite=$record["column_for_write"];
		$strThisBgClass=Alternate($strClassFirst, $strClassSecond, $strThisBgClass);
		$out.=htmlrow($strThisBgClass, "<a href=\"tableconcordance.php?" .qpre ."db=" . $strDatabase . "&concordance_id=" .  $concordance_id . "\">" . $concordance_id . "</a>",
			$table1,$field11, $field12,$table2, $fieldtowrite,
			"<a href=\"tableconcordance.php?" .qpre ."db=" . $strDatabase . "&" .qpre ."mode=edit&concordance_id=" .  $concordance_id . "\">edit</a>",
			"<a href=\"tableconcordance.php?" .qpre ."db=" . $strDatabase . "&" .qpre ."mode=view&concordance_id=" .  $concordance_id . "\">view</a>"
		);
	}
	$out.="</table>";
...
function FindConcordances($strDatabase, $table1, $table2, $field11, $field12, $field21, $field22, $fieldtoread, $fieldtowrite, $mode, $nooverwrite, $noexceedone,$noexceedonedest, $concordance_id="")
{
	...
	{
		$out.="<div class=\"feedback\">Concordances between tables with composite primary keys are not supported.</div>";
	}
	$out.= adminbreadcrumb(false, $strDatabase, "tableform.php" . "?" . qpre . "db=" . $strDatabase,  "table concordance", "tableconcordance.php?" . qpre . "db=" .$strDatabase, "scan results", "");
	$out.= " [<a href=\"tableconcordance.php?" . qpre . "db=" .$strDatabase . "\">new scan</a>]";
	if($concordance_id!="")
	{
		$out.= " [<a href=\"tableconcordance.php?" . qpre . "db=" .$strDatabase . "&" . qpre . "mode=rescan&concordance_id=" . $concordance_id . "\" onclick='return(confirm(\"A rescan can take several minutes.  Are you certain this is what you want?\"))'>rescan</a>]";
	}
	$out.="<br/>Looking for matches between source table <strong><a href=\"" . qbuild("tableform.php", $strDatabase, $pktable1, "view", "", "") . "\">" . $table1 . "</a></strong> and destination table <strong><a href=\"" . qbuild("tableform.php", $strDatabase, $table2, "view", $pktable2, $v) . "\">" . $table2 . "</a></strong>.<br/>";
	$out.="Looking for data narrowed by <strong>" . $field11 . "</strong> and <strong>" . $field12 . "</strong> that corresponds to data narrowed by <strong>" . $field21 . "</strong> and <strong>" . $field22 . "</strong>.";
	$out.= "<script src=\"tablesort_js.js\"><!-- --></script>";
	$tableheader="<table id=\"idsorttable\"  class=\"bgclassline\" cellpadding=\"0\" cellspacing=\"1\" border=\"0\" width=\"770\">\n";
	$out.= "<form method=\"post\" name=\"BForm\" action=\"tableconcordance.php\">\n";
	$out.= $tableheader;
	$out.=htmlrow("bglineclass", 
	...
	if(is_array($arrConcordance))
	{
		foreach($arrConcordance as $arrConcordanceHit)
		{
			...
			$thisatag1="<a target=\"newwindow\" href=\"" . qbuild("tableform.php", $strDatabase, $table1, "edit", $pktable1, $rpk1)  . "\">";
			$thisatag2="<a target=\"newwindow\" href=\"" . qbuild("tableform.php", $strDatabase, $table2, "edit", $pktable2, $rpk2)  . "\">";
			$thislashatag="</a>";
