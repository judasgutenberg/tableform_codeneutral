		...
		if ($intAdminType>1)
			{
				...
				$out =  PageHeader("table to table copy", $strConfigBehave) . $out . PageFooter();
				return $out;
			}
	}
}
...
function ShowSampleConcordanceData($strDatabase, $strTable1, $strTable2, $dests, $rec, $len=30)
{
	...
	$record1=$records1[0];
	$record2=$records2[0];
	$mrecords=DBExplain($strDatabase, $strTable1);
	...
	$out.="</table>\n";
	$urlpre="tableconcordanceviewer.php?" . qpre . "table1=" . $strTable1 . "&" .  qpre . "table2=" . $strTable2 . "&" .  qpre . "table2=" . $strTable2 . "&".   qpre . "dests=" . $dests; 
	if($rec>1)
	{
