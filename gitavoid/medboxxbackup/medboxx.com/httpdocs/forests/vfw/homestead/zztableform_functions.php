...
//i admit this function is becoming unmaintainable due to the many scenarios i have forced it to handle
{
	...
	if ($displaytype!="")   //it's a graph
	{
		...
		else
		{
			...
			if ($FKtable!="")
			{
				...
				$descr=DBExplain($strDatabase, $FKtable); 
	...
	$strDoSQLAdditional.=" LIMIT " . $intRecord . "," . $intRecsPerPage;
	...
	foreach ($records as $key=>$value )
	{
		...
		foreach ( $value as $key1 => $value1 )
		{
			...
			if (GreaterFieldDisplayLogic($strTable, $strFieldConfig, $key1, $intFieldCount, $intFieldLimitLo, $intFieldLimitHi, $display_field_list) && !$bwlForceHide || ($strSearchField==$key1))
			{
				...
				if ($bwlBrowseMode && $strNameColumn== $key1)  //we're in browser mode and this is a name column, so time for a hyperlink
				{
					...
					$strDataToDisplay="<a href=\"" . qbuild($strPHP, $strDatabase, $strOriginalTable , "view", $strIDFieldName, $value[$strIDFieldName]) . "&" . qpre . "displaytype=" . $displaytype . "&" . qpre . "filterid=" . $value[$strIDFieldName] .  $keyserialized . "\">" . trunchandler( $value1, $intTruncSize) . "</a>";
		...
		if ($bwlShowEdit)
		{
			...
			{
				$out.= "<a style=\"" . $strActionStyle . "\" href='" . qbuild($strPHP, $strDatabase, $strTable, "edit", $strIDFieldName, $value[$strIDFieldName])  . "&" . qpre . "displaytype=" . $displaytype  ."&" . qpre . "filterid=" . $intThisFilterID . "&" . qpre . "rec=" . $intRecord . "&" . qpre . "column=" . $strSortColumn  . "&" . qpre . "direction=" . $strDirection . $keyserialized . "'>Edit</a>";
			}
...
//breaking this out into a separate function probably wasn't the wisest idea, but i had to get stuff out of DisplayDataTable
{
	...
	if (!$bwlSuppressHeaderLinks  || $bwlAlwaysAllowNewLink)
	{
		...
		$out3.= "<a href='" . qbuild($strPHP, $strDatabase, $strTable, "new", "", "")  . "&" . qpre . "displaytype=" . $displaytype  . "&" . qpre . "filterid=" . $intThisFilterID . "'>New <b>" .  forceSingular($strTable) . "</b></a>";
		//$out3.= "</td>\n";
	}
	//$out2.= "</tr>\n</table>\n";
...
//this system will automatically scan through the list of files given in the ddl called $strConfig, labeling them with the second fields in that ddl.
	...
	$strConfig="tableform.php|Tableform-core_functions.php|Core function library-tableform_js.js|Core javascript  library-tablemaker.php|Tablemaker-tablemaker_js.js|Tablemaker javascript-saveform.php|Tableform saver library-dbtools.php|Database tools-dump.php|CSV tools-backup_functions.php|Backup functions-frontenddbfunctions.php|Frontend db functions-genericuserfunctions.php|Generic user functions";
	$versionconfig="0.5|1182650153-1|1191945058-1.1|1196911057-1.2|1228931859 ";
	$date=filemtime("core_functions.php");
	...
	$out="";
	$out.="<table cellpadding=\"2\" cellspacing=\"1\" border=\"0\" width=\"450\" class=\"bgclassline\">\n";
	...
	foreach ($descr as $nom=>$info)
		{
				...
				if (!$bwlInvisible)
				{
					...
					if ($strType=="int")
					{
						...
						if (count($arrMK)>0)
						{
							...
							//	contains the valuea of foreign PKs.
				...
				elseif ((($strType=="tinyint"  || $arrForce_binary[$nameforhelp]) || $strType=="bit") && !$arrForce_text_input[$nameforhelp]  &&  !(abs(intval($strDefault))>1))
				{
...
//shows all the tables in $strDatabase
{
	...
	//$out.= AdminNav($bwlSuperAdmin);
	$out.= "<script src=\"" . tf_dir . "tablesort_js.js\"><!-- --></script>";
	...
	foreach ( $tables as  $k=>$v )
	{
		...
		if (($bwlBeginsWithTF &&  $bwlSuperAdmin) || !$bwlBeginsWithTF)
		{
			...
			{
				$tablemakerlink= "[<a href=\"" . qbuild("tablemaker.php", $strDatabase, $tablename , "", "", "") . "\">edit def.</a>]";
			}
			else
			{
				$tablemakerlink= "&nbsp;";
...
function TableForm($strDatabase, $strTable, $strIDField, $strValDefault, $strPHP,  $strConfigBehave, $strFieldNamePrefix="", $strUser="",  $arrPK="", $strDisplayMode="", $strKosherFields="")
{
...
//a generic form generator that looks at the table's description in the db and then dynamically builds an editor form
	...
	if (!contains($strConfigBehave, "noheader")  && !contains($strConfigBehave, "notopnav"))
	{
		...
		$out.=adminbreadcrumb($disablelink,  $secondarytoollable, $strDBLink,  $strTable, qbuild($strPHP, $strDatabase, $strTable, "view", "", "")) . $closebutton;
				...
				else
				{
					...
					else if (inlist("bigint mediumint int smallint", $strType))
					{
						...
						if (count($arrMK)>0)
						{
							...
							$strDistantTable=GenericDBLookup($strDatabase, $strRTable, $strIDFieldName, $arrRelationTemp[$strRTable], $strRField);
							//echo "d:" . $strDistantTable . " idfn: " . $strIDFieldName . " rt:" . $strRTable  . " rfield: " . $strRField . "<br>";;
							$intDistantPK = PKLookup($strDatabase, $strDistantTable);
							$intNestTable++;
							$strNest.="|" . $intNestTable;
							//just for shits im gonna get all recursive right now...
							...
							$futurelabel=$strDistantTable;
							$futureid=$strDefault;
							//2009 COMMENTARY: IT WOULD BE AWESOME IF THE FOLLOWING PULLDOWN COULD BE AJAX-altered BY THE PULLDOWN OF THE PRECEDING TYPE TABLE
...
{
	$out="<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" width=\"840\">\n";
