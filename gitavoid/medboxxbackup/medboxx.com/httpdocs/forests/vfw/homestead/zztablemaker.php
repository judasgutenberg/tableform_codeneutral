			...
			if ($intAdminType>1)
				{
					...
					$out.=IfAThenB($possibleerrors, "<div class=\"feedback\">" . $possibleerrors . "</div>");
					
				}
		}
		$out =  PageHeader($strDatabase . IfAThenB($strTable, " : ") . $strTable . " : Table Maker", $strConfigBehave ) .
			"<script src=\"tablemaker_js.js\"><!-- --></script>" . 
		$out . PageFooter();
...
function TableMaker($strDatabase, $strTable, $strTypeConfig, $bwlAdvanced=false)
{
	...
	if($strTable=="")
	{
		$out.= adminbreadcrumb(false,  $strDatabase, "tableform.php?" . qpre . "db=" . $strDatabase,  "New Table");
	}
	else
	{
		$out.= adminbreadcrumb(false,  $strDatabase, "tableform.php?" . qpre . "db=" . $strDatabase,  "altering table definition for " . $strTable, "");
	}
	//$out.= AdminNav(true);
	$out.="<form name=\"BForm\" method=\"post\" action=\"" . $strPHP . "\" onSubmit=\"  \">\n";
	$out.= "<table id=\"idsorttable\" border=\"0\" cellspacing=\"1\" cellpadding=\"2\" class=\"" .$strLineClass  . "\" width=\"650\">\n";
	...
	$out.="<tr class=\"bgclassother\"><td colspan=\"10\"    align=\"left\">" .  $strTableNameLabel . ": <input type=\"text\" size=\"22\" value=\"" .  $strTable . "\" name=\"" . qpre . "table\" />\n";
	if ($strTable!="")
	{
		$count = countrecords($strDatabase , $strTable );
		$out.="[<a href=\"" .  qbuild("tableform.php", $strDatabase, $strTable, "view", "", "")  . "\">view " . $count ." " . PluralizeIfNecessary("record", $count) . " of existing table data</a>]";
		$out.=" [<a href=\"" .  qbuild("dbtools.php", $strDatabase, $strTable, "entersql", "", "") ."&" . qpre .  "actscript=show+create+table+"  . $strDatabase . "." . $strTable  . "\">view SQL table definition</a>]";
		$out.= " [<a href=\"tablemaker.php\">new table</a>]";
	}
	else
	{
		$out.= " [<a href=\"" . qbuild("import.php", $strDatabase, $strTable, "", "", "") . "\">create table from imported data</a>]";
	
	}
	$out.="\n</td>\n</tr>\n";
	if($bwlAdvanced)
	{
		$out.=htmlrow("bgclassline", "select", "column", "type", "size","default", "nullable","&nbsp;PK", "&nbsp;auto++", "&nbsp;more","&nbsp;+field" );
	}
	else
	{
		$out.=htmlrow("bgclassline", "select", "column", "type", "size","", "","&nbsp;PK", "&nbsp;auto++", "&nbsp;more" ,"&nbsp;+field" );
	}
	...
	$out.=FieldEditLine($strDatabase, $outcount,"","","", "", true,false,  false, "", "",  "", "", "", "", "", "", "", "", "", "", "", $strTypeConfig,$strUpDownArrows,"hideallbutlast", $strStyleAdvanced,   $autoinccheckboxstyle, $strStyleAdvanced);
		...
		foreach ($descr as $nom=>$info)
			{
					...
					$strUpDownArrows=UpDownArrows($outcount);
					$out.=FieldEditLine($strDatabase, $outcount,$name,$strType,$intLen, $default, $bwlNull,$bwlPK,  $bwlAI,  $friendly, $helptext, $validation_type_id,$validation_pattern_id, $invisible, $force_wysiwg, $force_no_wysiwg, $force_binary, $force_text_input, $fileupload, $password, $width, $height, $strTypeConfig, $strUpDownArrows, $strThisBgClass, $strStyleAdvanced,   $autoinccheckboxstyle, $strStyleAdvanced);
...
function AlterTable($strDatabase, $strTable,  $strOldTableName,  $strTypeConfig)
{
	...
	for($i=0; $i<=$count; $i++)
	{
		...
		if($helptext!=""  || $friendly!="" || $validation_type_id!=""  || $invisible!="" || $force_no_wysiwg!="" || $force_wysiwg!=""  || $force_no_bianry!="" || $force_binary!="" || $fileupload!="" || $password!="" || $validation_pattern_id!="" || $width!=""  || $height!="")
		{
...
function RelatedTables($strDatabase, $strTable)
{
	...
	foreach($records as $record)
	{
		...
		$out_f.="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"tablemaker.php?" . qpre . "db=" . $strDatabase . "&" . qpre . "table=" . $f_table_name . "\">" . $f_table_name . "</a><br/>";
	}
	...
	foreach($records as $record)
	{
		...
		$out_p.="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"tablemaker.php?" . qpre . "db=" . $strDatabase . "&" . qpre . "table=" . $table_name . "\">" . $table_name . "</a><br/>";
	}
	if ($out_f!="")
	{
		$out.="<td valign=\"top\">\n";
	...
	if ($out!="")
	{
		$out="<table width=\"400\" cellpadding=\"0\" cellspacing=\"0\"><tr><td class=\"heading\">Related Tables</td></tr><tr>" . $out . "</tr></table>";
	
	}
	return $out;
}

function TypeSQL($strType, $strSize)
{
	...
	if (!inList("mediumtext time longtext datetime text date timestamp", $strType))
	{
		$out= $strType . "(" . $strSize . ") ";
	}
	else
