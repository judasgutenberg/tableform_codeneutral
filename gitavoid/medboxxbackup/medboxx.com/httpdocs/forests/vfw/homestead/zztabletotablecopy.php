		...
		if ($intAdminType>1)
			{
				...
				{
					$out.= adminbreadcrumb(false, $strDatabase, "tableform.php" . "?" . qpre . "db=" . $strDatabase,"db tools","dbtools.php",  "table to table copy", "tabletotablecopy.php", "create regime or use existing regime", "");
					$out.="<p/>\n";
...
function PickTables($strDatabase, $strTable1, $strTable2)
{
	...
	$out.="<form name=\"BForm\" action=\"tabletotablecopy.php\" method=\"post\">\n";
...
function LinkFields($strDatabase, $strTable1, $strTable2, $tablecopymap_id, $bwlMerge)
{
	...
	if($strRegimeName!="")
	{
		$out.= adminbreadcrumb(false, $strDatabase, "tableform.php" . "?" . qpre . "db=" . $strDatabase,"db tools","dbtools.php",  "table to table copy", "tabletotablecopy.php?tablecopymap_id=" . $tablecopymap_id, $strRegimeName, "", $strBreadcrumblabel, "");
	}
	else
	{
		$out.= adminbreadcrumb(false, $strDatabase, "tableform.php" . "?" . qpre . "db=" . $strDatabase,"db tools","dbtools.php",  "table to table copy", "tabletotablecopy.php?tablecopymap_id=" . $tablecopymap_id, $strBreadcrumblabel, "");
	}
	$out.="<p/>\n";
	$out.="<a href=\"javascript:concordanceviewer()\">view sample data for this regime</a>";
	$out.="<div class='heading'>Map columns of source " . $strTable1 . " to columns of destination " . $strTable2 . "</div>";
	$out.="<form name=\"BForm\" action=\"tabletotablecopy.php\" method=\"post\">\n";
	...
	$i=0;
	$out.="</table>";
	$out.="<table id=\"idsorttable\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\">\n";
	...
	$records=DBExplain($strDatabase, $strTable1);
	...
	$out.= "<script src=\"tabletotableconcordanceviewer.js\"><!-- --></script>";
...
//GenericRSDisplay($strDatabase, $strPHP,$strLabel, $strSQL, $truncate, $intWidth, $strLinkFieldName="", $strLinkIDName="", $strAdditionalLink="", $strSuppressFields="", $bwlSuppressHeader=false, $bwlPrettyUpFieldNames=true)
	...
	if($strRegimeName!="")
	{
		$out.= adminbreadcrumb(false, $strDatabase, "tableform.php" . "?" . qpre . "db=" . $strDatabase,"db tools","dbtools.php",  "table to table copy", "tabletotablecopy.php", $strRegimeName, "tabletotablecopy.php?" . qpre . "mode=edit&tablecopymap_id=" . $tablecopymap_id, $strBreadcrumblabel, "");
	}
	else
	{
		$out.= adminbreadcrumb(false, $strDatabase, "tableform.php" . "?" . qpre . "db=" . $strDatabase,"db tools","dbtools.php",  "table to table copy", "tabletotablecopy.php", $strBreadcrumblabel, "");
	}
	$out.="<p/>\n";
	...
	$out.="<form name=\"BForm\" action=\"tabletotablecopy.php\" method=\"post\">\n";
	$out.=GenericRSDisplay($strDatabase, "tabletotablecopy.php","Select Rows to Copy from " . $strTable1 . " to " . $strTable2, $strSQL, 1, 850, "", "", $checkcell, "", false, true);
	$out.= HiddenInputs(array("db"=>$strDatabase  ));
	$out.= HiddenInputs(array("table1"=>$strTable1, "table2"=>$strTable2 ));
	$out.= HiddenInputs(array("tablecopymap_id"=>$tablecopymap_id), "");
...
function OtherLinkRegimeList($strDatabase)
{
	...
	$checkboxtag.=" [<a href=\"tabletotablecopy.php?" . qpre . "mode=transpose&tablecopymap_id=<replace/>\">create transposition</a>]";
	$checkboxtag.=" [<a href=\"tabletotablecopy.php?" . qpre . "mode=edit&tablecopymap_id=<replace/>\">edit</a>]";
	$checkboxtag.=" [<a href=\"tabletotablecopy.php?" . qpre . "mode=selectrows&tablecopymap_id=<replace/>\">use</a>]";
	$out.=GenericRSDisplay($strDatabase, "tabletotablecopy.php?" . qpre . "mode=edit","Select an existing copy regime:",$strSQL, true, "100%", "tablecopymap_id", "tablecopymap_id", $checkboxtag);
	$out.= HiddenInputs(array("db"=>$strDatabase  ));
	$out.= HiddenInputs(array("table1"=>$strTable1, "table2"=>$strTable2 ));
	$out.= HiddenInputs(array("tablecopymap_id"=>$tablecopymap_id), "");
