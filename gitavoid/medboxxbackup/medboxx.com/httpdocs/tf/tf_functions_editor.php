	...
	foreach ($records as $key=>$value )
	{
		...
		foreach ( $value as $key1 => $value1 )
		{
			...
			if (GreaterFieldDisplayLogic($strTable, $strFieldConfig, $key1, $intFieldCount, $intFieldLimitLo, $intFieldLimitHi, $display_field_list) && !$bwlForceHide || ($strSearchField==$key1))
			{
				...
				$strPrettyName=LabelThis("", $strNameColumn, $connectorreturn, $value, "", $intTruncSize);
				...
				else if (inlist("bigint mediumint int smallint", $strType) || $strType=="tinyint" && endswith($name, "id"))
				{
					...
					if (count($arrFK)>0)
					{
						...
						if ($count<101)
						{
							...
							$fieldform=foreigntablepulldown($strDatabase,$arrFK[0], $arrFK[1], $strDefault, $strFieldNamePrefix .$name, $bleh, false, $arrFK[4], "", $arrFK[6],$record);
							//idl-
						...
						else
						{
							...
							$strPickerLabel=LabelForID($strDatabase, $arrFK[0], $arrFK[1], $strDefault,$arrFK[4]);
							$strPickerLink= "<input style=\"font-size:9px; border:0px;\" class=\"" . $strThisBgClass . "\" type=\"text\" name=\"" . $strFieldNamePrefix .  qpre . "a|" .  $name . "\" size=\"20\" value=\"" . deescape($strPickerLabel)  . "\">\n";
							$strPickerLink.=" [<a href=\"#\" onclick=\"pickerwindow('". $strDatabase . "','" . $arrFK[0] . "','".  $strFieldNamePrefix . $name ."','','','" . $strTable . "','" . urlencode(serialize($arrUniversalPK)) . "')\">browse</a>]\n";
