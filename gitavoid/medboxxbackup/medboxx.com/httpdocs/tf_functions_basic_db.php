...
//this function has pretty much replaced my use of explicit SQL for updates and inserts, though Saveform still does things that way
{
	...
	else if (count($records)<1  ||  !is_array($arrDescribedValuePairs)) //this code won't allow a new record that is identical except for pk. may need to think about it some more
	{
		...
		$arrValuelist=JoinList(",",$strValueList ,$strValueListDesc);
		if(count($arrValuelist)>0)//don't insert empty rows!
		{
			//echo "$";
			$strSQL="INSERT INTO " . $strDatabase . "." .  $strTable . "(" . JoinList(",", $strColumnList, $strColumnListDesc) . ") VALUES(" . $arrValuelist  . ")";
		}
	...
	if($strSQL!="")
	{
		...
		if(CanChange())
		{
	 		//echo mysql_error();
			$records = $sql->query($strSQL);
			//echo $strSQL  . "<br>" .  mysql_error() . "^^<BR><HR>";
...
{
	$sql=conDB();
	$strSQL="SELECT COUNT(*) FROM " . $strDatabase . "." . $strTable . " " .  MergeSQLConditionals(MergeLimitingConditionsWithRSData($strWhereClause, $recordforusewithwhereclause) , "");
	//echo $strSQL . "<BR>";
...
function DeleteBySpec($strDatabase, $strTable, $arrSpec, $bwlDontFork=false)
{
	$sql=conDB();
	$strWhereClause=ArrayToWhereClause($arrSpec);
	$strDeleteSQL="DELETE FROM " . $strDatabase . "." .  $strTable . " WHERE " . $strWhereClause;
