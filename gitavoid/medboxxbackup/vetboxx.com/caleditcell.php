...
else
{
	...
	{	
		...
		if($appointment_duration!="" && $client_id!="" && $practitioner_id!="" &&( $mode=="save" || $mode=="another"))
		{
			...
			//die();
			//die(date("Y-m-d H:i:s",$dtmNow). " " .  makedatecode($dtmNow));
			...
			if($calendar_event_id!="")
			{
				...
				ScheduleAPhoneCall($calendar_event_id, $no_phone);
			}
			$out=EndLayerScript($practitioner_id);
		}
		else if($mode=="save")
		...
		{
			$strHidden="guardian_client_id city name_of_guardian subdomain zip office_name password address state_code client_type_id login_id client_id";
			$labels["client_id"]="";
			if(strtolower(sitename)=="vetboxx.com")
			{
			...
			$results=GenericRSDisplay($strDatabase, "",$strLabel, $strSQL, $truncate, $intWidth,  "client_id",  "client_id",  " <a target='new' href=\"view.php?client_id=<replace/>\">edit</a> |  <a href=\"javascript:selectuser('<replace/>','<firstname> <lastname>')\">select</a>",  $strHidden,  false, true,  12, "", "", $labels);
			if($results=="")
			{
		...
		{
			$arrButtons=Array("*onClick=JSEndLayerScript()"=>"Don't Save","*onClick=if(document.CForm.practitioner_id[document.CForm.practitioner_id.selectedIndex].value==\"\"){alert(\"You must select a practitioner.\");return false}else{document.AForm.submit()}"=>"Schedule" );
			if($calendar_event_id!="")
			{
				$arrButtons["unschedule"]="Unschedule";
			}
			$arrHidden["client_id"]=gracefuldecay($arrHidden["client_id"], $client_id);
 			$out.=  GenericForm(Array("patient_name", "appointment_duration", "notes", "no_phone", "event_status|event_status_id|status_name|event_status_id|"),  Array(  $strCustomerNoun ." Name",   "Appointment Duration", "Notes", "Do Not Phone", "Appointment Status"), $arrHidden, Array("no_phone"=>"type:checkbox","appointment_duration"=>"dropdown:5|5&nbsp;minutes-10|10&nbsp;minutes-15|15&nbsp;minutes-20|20&nbsp;minutes-30|30&nbsp;minutes-45|45&nbsp;minutes-60|one&nbsp;hour-90|90&nbsp;minutes-120|2&nbsp;hours-180|3&nbsp;hours-240|4&nbsp;hours-300|5&nbsp;hours-360|6&nbsp;hours", "notes"=>"size:2030"), $strPHP,  "", "100%",  "", $formextratags, "AForm", false, $arrButtons);
		}
