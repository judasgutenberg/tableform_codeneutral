...
//Looks up the multi-table info given a column
{
		...
		foreach ($records as $record)
		{
			if($mode=="")
			{
				...
				return array( $record["f_table_name"],  $record["f_column_name"], $record["table_name"], $record["column_name"], $record["display_column_name"], $record["tool_guidance_id"], $record["narrowing_conditions"]) ;
			}
