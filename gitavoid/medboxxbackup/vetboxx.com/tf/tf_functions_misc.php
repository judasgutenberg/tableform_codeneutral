...
function PageHeader($strTitle, $strConfigBehave,$strForBackField="", $bwlIsStandalone=true, $bwlSuppressExternalHeader=false, $intMetaRefresh="")
...
function GenericForm($arrFieldNames,  $arrLabels, $arrDefaults, $arrFieldConfigs, $strPHP, $strButtonLabel="Save", $width=630, $arrSizes="", $formextratags="", $formname="BForm", $bwlAllOneRow=false, $arrExtraButtons="")
...
//although this thing is pretty complicated now!
{
	...
	for($i=0; $i<count($arrFieldNames); $i++)
	{
		...
		else
		{
			...
			if($bwlAllOneRow)
			{
				$out.= gracefuldecay($foundlabel,$arrLabels[$i], $thisfieldname) . "&nbsp;" .  $inputtag . $infostring;
			}
			else
			{

				$out.=htmlrow($classToUse, gracefuldecay($foundlabel, $arrLabels[$i], $thisfieldname), $inputtag . $infostring);
			}
		}
	...
	if($bwlAllOneRow)
	{
		$out.="&nbsp;" . $strExtraButtons . GenericInput(qpre . "save", $strButtonLabel, "","","",  "genericbutton");
	}
	else
	{
		$out.=htmlrow("^2" . $strOtherBgClass,  "<div align='right'>" . $strExtraButtons .  GenericInput(qpre . "save", $strButtonLabel, "","","",  "genericbutton"). "</div>");
	}
	$out.= "</form>\n";
 	if($bwlAllOneRow)
	{
...
function LabelForID($strDatabase, $strTable, $strIDField, $intID, $strKnownFields="")
{
	if($strKnownFields=="")
	{
		...
		$arrNameFields=Array($strNameField);
	}
	else
	{
	
		if(contains($strKnownFields, " "))
		{
			$arrNameFields=explode(" ",  $strKnownFields);
			$connector=" ";
		}
		else
		{
...
function foreigntablepulldown($strDatabase, $strTable, $strIDField, $intDefault, $strLabelField="", &$namereturn, $bwlHiddenReturn=false, $strPreferredNameField="", $addedselectpairs="", $strWhereClause="")
...
//THIS FUNCTION IS BEGGING TO BE REFACTORED!!!
{
	...
	else
	{
		...
		{
			$arrPreferredNameField=explode(" ",$strPreferredNameField);
			$strNameField=$arrPreferredNameField[0];
			$strNameField2=$arrPreferredNameField[1];
			$orderby=$strNameField2;
			$bwlForceBothFields=true;
			$connector=" ";
			//echo $possiblelabel2field . "=<BR>";
	 
		}
		else if(contains($strPreferredNameField, ","))
		{
			$arrPreferredNameField=explode(",",$strPreferredNameField);
			$strNameField=$arrPreferredNameField[0];
			$strNameField2=$arrPreferredNameField[1];
			$orderby=$strNameField1;
			$bwlForceBothFields=true;
			$connector=", ";
		}
		else if($strPreferredNameField!="")
		{
	...
	if ($records)
	{
		...
		foreach($records as $k)
		{
			...
			if ((strlen($strLabel1)<15 && $strLabel2!="" && !(strlen($strLabel2)>15))   && ($strNameField2!="password") || $moreflag  || $bwlForceBothFields)
			{
				...
				else if(trim(truncate($strOldLabel, 35))==trim(truncate($strLabel1, 35)) )
				{
