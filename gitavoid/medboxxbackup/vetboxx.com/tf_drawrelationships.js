...
{ 
	...
	thisitem.style.background='#ff0000';
	globalidstart=id;
	...
	//alert("#" + globalidstart);
	setTimeout('popupcolumndialog();',700);
...
function popuptabledialog(id)
{
	...
	var arrAnchortext= new Array('edit table');
	var arrLink= new Array('javascript:globalpopupmode="edittable"')
	PopupMenuAtMousePosition(arrAnchortext, arrLink, "idmapdialog");
}

function PopupMenuAtMousePosition(arrAnchortext, arrLink, id)
{
	//alert(globalx + " " + globaly);
	...
	div.style.border="0px";
	div.style.position="absolute";
	div.style.left=globalx;
	div.style.top=globaly;
	...
	for(i=0; i<arrAnchortext.length; i++)
	{
		...
		thislink.setAttribute("onmouseover", arrLink[i]);
		thislink.setAttribute("onmouseout", 'javascript:globalpopupmode=""')
		thislink.setAttribute("class", 'mapfield');
		thislink.innerHTML=arrAnchortext[i];
		var br=document.createElement('br');
		var ae=document.createElement('a');
		div.appendChild(thislink);
		div.appendChild(br);
	}
...
function popupcolumndialog()
{
	if(globalsavedx==globalx  && globalsavedy==globaly)
	{ 
		var arrAnchortext= new Array('delete relation', 'edit relation');
...
{ 
	...
	if(globalBwlMakingRelation  && id!=globalidstart)
	{
		...
		thisitem.style.backgroundColor='#3333ff';
		globalidhovering=id;
		//alert("-" + globalprevbg);
		//alert(thisitem.style.background);
	}
