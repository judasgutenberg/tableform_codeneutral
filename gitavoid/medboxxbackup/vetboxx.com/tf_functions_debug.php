...
//logs the serialized request object;  used for debugging or seeing what exactly an idiot user is doing that keeps fucking things up
	...
	if(TableExists($strDatabase, $strTable))
	{
		...
		UpdateOrInsert($strDatabase, $strTable, "", Array("comments"=>$comments, "record_label"=>$label,"referer"=>$referer, "ip_address"=>$_SERVER['REMOTE_ADDR'], "time_done"=> date("Y-m-d H:i:s"), "form_content"=>$content), true, false, true);
	}
}
