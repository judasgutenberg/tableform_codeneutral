...
function AlterTable($strDatabase, $strTable,  $strOldTableName,  $strTypeConfig)
{
	...
	for($i=0; $i<=$count; $i++)
	{
		...
		$whereclause=" table_name='" . AppropriateSQLName($strTable) . "' AND column_name='" .  ensureEnclosure(AppropriateSQLName($thisField), "`", "`")  . "'";
		//this code may need to be changed!!!!
		if($extra_functionality!="" || $helptext!=""  || $friendly!="" || $validation_type_id!=""  || $invisible!="" || $force_no_wysiwg!="" || $force_wysiwg!=""  || $force_no_bianry!="" || $force_binary!="" || $fileupload!="" || $datecreated!="" || $datemodified!="" || $password!="" || $validation_pattern_id!="" || $width!=""  || $height!="")
		{
			...
			$strPostExtraSQL= $strDatabase . "." .  tfpre . "column_info SET ";
  			$strPostExtraSQLValues=IfAThenB($helptext,",help_text='" . escapeifneeded($helptext) . "'") . 
			IfAThenB($extra_functionality,",extra_functionality='" . escapeifneeded($extra_functionality) . "'") . 
