		...
		else
		{
			...
			if($action=="viewdependents")
			{
				...
				if(strtolower(sitename)=="vetboxx.com") 
				{
					...
					{
						$searchWhere =MergeSQLConditionals($searchWhere, " guardian_client_id='" . $_REQUEST["client_id"] . "'", FALSE) ;
						 
					}
					else
					{
						$searchWhere =MergeSQLConditionals($searchWhere, "", false);
					}
					$strSQL="SELECT client_id, firstname  , client_description as 'type' FROM " . our_db . ".client c LEFT JOIN  " . our_db . ".client_type t ON c.client_type_id=t.client_type_id  WHERE  " .  $searchWhere . " ORDER BY lastname, firstname";
				}
				else
				{
				...
				$labels=Array("client_id"=>"","datecode"=>"date", "name"=>strtolower(professionaltype));
				//echo $strSQL;
				//echo mysql_error();
