...
{ 
	...
	if(thisitem)
	{
		var thisParent=GetParent(thisitem);
		thisParent.removeChild(thisitem);
	}
...
function NoTablePopup()
{
	...
	//var arrLink= new Array
		...
		//'javascript:addtable()'
	//	)
	PopupMenuAtMousePosition(arrAnchortext, arrLink, "idnotable", "onmouseup", 120);
...
function PopupMenuAtMousePosition(arrAnchortext, arrLink, id, actiontype, width)
{
	...
	PopupMenuAtPosition(arrAnchortext, arrLink, id, globalx, globaly, width,  "cccccc", actiontype)
}
...
function edittable(id, thistype)
{
	...
	{
		editselected("", tfpre + "table_maker.php?" + qpre + "table=" + thistable + "&" + qpre + "db=" + database)
	}
