...
function SpecificCalendarDayIframe($datecode)
{
	$out.="\n<iframe frameborder=\"0\" marginwidth=\"0\" marginheight=\"0\" width=\"800\" height=\"600\" name=\"dayframe\" src=\"dayframe.php?datecode=" . $datecode . "\"></iframe>";
	return $out;
}
...
function CalendarDataFrame($datecode)
{
	$out.="\n<iframe frameborder=\"0\" marginwidth=\"0\" marginheight=\"0\" width=\"1\" height=\"1\" name=\"calendardataframe\" src=\"calendardata.php?datecode=" . $datecode . "\"></iframe>";
	return $out;
} 


function CalendarData($datecode,  $intUserID, $strPHP)
{
	$sql=conDB();
	$out="";
...
$strSQL="create temporary table " . our_db . ".results(datecode varchar(50), counter int, name varchar(50), result_type int, recurrence varchar(30) );";
	$records = $sql->query($strSQL);
		//echo mysql_error()."\n\n\n\n";
$strSQL="insert into " . our_db . ".results(datecode, counter, name, result_type, recurrence) select c.datecode, count(*) as counter, p.name ,1, recurrence_interval  FROM  " . our_db . ".personal_calendar_event c LEFT JOIN  " . our_db . ".practitioner p ON c.practitioner_id=p.practitioner_id  LEFT JOIN  " . our_db . ".recurrence r on c.recurrence_id=r.recurrence_id WHERE  c.user_id=" . $intUserID . " GROUP BY datecode;";
	$records = $sql->query($strSQL);
		//echo mysql_error()."\n\n\n\n";
$strSQL="insert into " . our_db . ".results(datecode, counter, name, result_type) select concat( SUBSTRING(year(date_time),3,2),lpad(month(date_time),2,'0') , lpad(dayofmonth(date_time),2,'0')), 1 ,p.name, 2 from " . our_db . ".medical_date m join  " . our_db . ".practitioner p on m.practitioner_id=p.practitioner_id where user_id=" . $intUserID . " ; ";
	$records = $sql->query($strSQL);
		//echo mysql_error()."\n\n\n\n";
$strSQL="select  datecode, name, sum(counter) as counter, sum(result_type) as type, recurrence  from " . our_db . ".results group by datecode order by datecode";
...
function MultipleEventList($strDatabase, $datecode, $intUserID,  $strSearchTerm="", $strSearchType="", $strFTables)
{
	...
	foreach ($records as $k => $v )
	{
	 	...
	 	if ((contains($v["Type"], "varchar") || $v["Type"]=="text")  && $v["Field"]!="datecode" )
		{
			...
			$arrLabels[$intFieldOut]="<label>" . DisplayLabel($question) . "</label>" . answerformwrap("", $question_id, 0, $sql_data_type, $foreigntable, true, $strWidth, $intHeight, $intUserID, $false, $strTable);
	...
	$additionalClauses=" WHERE datecode=" .$datecode . " AND t.user_id=" . $intUserID . " ORDER BY sort_id ASC";
	$strSQL=FleshedOutFKSelect($strDatabase, $strTable, $additionalClauses,  $arrDesc, "bb_user", true);
	//echo "<b>eventsql:</b> " . $strSQL . "<P>";
	$records = $sql->query($strSQL);
	//echo mysql_error();
	$out.= "<script src=\"multipleanswer_js.js\"><!-- --></script>\n";
	$out.= "<script src=\"tableform_js.js\"><!-- --></script>\n";
	$out.= "\n<script>	colorbg('95a8fE')</script>\n";
	...
	for($i=1; $i<=$sort_id; $i++)
	{
		...
		for($j=1; $j<count($arrQuestionIDs) ; $j++)
		{
			...
			foreach($arrFTables as $strFTable)
			{
				if ($arrFieldNames[$j]==$strFTable . "_id")
				{
					$foreigntable=$strFTable;
				}
				else if ($arrFieldNames[$j]== "recurrence_id")
				{
					$foreigntable="recurrence";
				}
			}
			...
			//echo $answer . "<br>";
			$arrOut[$j]=answerformwrap($answer, $intQuestionID, $i,$arrDataTypes[$j],  $foreigntable, false, $arrWidths[$j], $arrHeights[$j], $intUserID, $false, $strTable);
...
function MultipleEventListReadonly($strDatabase, $datecode, $intUserID, $strNoUserMessage="", $strSearchTerm="", $strSearchType="", $bwlSuppressSearch=false,  $strFTables="", $layoutwidth="100%", $bwlNoForm=true)
{
	...
	if($intUserID!="")
	{
		...
		if($strSearchTerm!="")
		{
			...
			if ($strSearchType=="")
			{
				$searchstring=" (start LIKE  '%" .  $strSearchTerm . "' OR end LIKE '%" .  $strSearchTerm . "%' OR name LIKE  '%" .  $strSearchTerm . "%' OR type LIKE  '%" .  $strSearchTerm . "%' OR notes LIKE  '%" .  $strSearchTerm . "%') ";
		...
		$strSQL=FleshedOutFKSelect($strDatabase, $strTable, $strWhere,  $arrDesc, "bb_user", false);
		//echo "<b>eventsql:</b> " . $strSQL . "<P>";
		$records = $sql->query($strSQL);
		//echo count($records);
	 
		$out.= "<form method=\"post\" name=\"BForm\" action=\"" .  $strPHP . "\">\n";
		$out.= "<table border=\"0\" cellspacing=\"1\" cellpadding=\"2\" class=\"" .$strLineClass  . "\" width=\"" . $layoutwidth . "\">\n";
		//$arrParam[0]="";  //because of the way htmlrow works, this is to set the class of the display of that row
		...
		if ($bwlNoRecords)
		{
			if($strSearchTerm=="")
			{
				$out="<span class=\"text_12b2\">There is nothing in My Calendar for today.</span>";
			}
			else
			{
				$out="<span class=\"text_12b2\">No match was found for your search.</span>";
			}
...
function SaveEventList($strDatabase, $intUserID, $datecode, $strFTables)
{
	$sql=conDB();
	...
	if($fieldcount>0)
	{
		$strSQL="SELECT MAX(sort_id) as mgid FROM " . $strDatabase . "." . $strTable . " WHERE datecode='" . $datecode . "' AND user_id=" .$intUserID;
		$records = $sql->query($strSQL);
		$record=$records[0];
		...
		if($intMaxGroupID>$fieldcount)
		{
			for($i=$fieldcount; $i<$intMaxGroupID; $i++)
			{
				$strSQL="DELETE FROM " . $strDatabase . "." . $strTable . " WHERE datecode='" . $datecode . "' AND user_id=" .$intUserID . " AND sort_id='" . $i . "'";
				//echo "<br>" . $strSQL . "<br>";
				$records = $sql->query($strSQL);
			}
		}
		...
		for($intY=1; $intY< $fieldcount+1; $intY++)
		{
			...
			for($j=0; $j<count($arrX); $j++)
			{
				...
				if($intX!="")
				{
					...
					$arrSlurpedData[$arrFieldNames[intval($j)+1]]=$v;
					if($v!="")
					{
						$allempty=false;
					}
			...
			if( !$allempty)
			{
				...
				UpdateOrInsert($strDatabase, $strTable, Array("user_id"=>$intUserID,"datecode"=>$datecode, "sort_id"=>$intY), $arrSlurpedData, false);
				$event_name="";
				$type="";
				$notes="";
			}
...
function CalSearchForm($strDatabase, $strPHP, $strSearchTerm, $strTypeDefault)
{
	...
	$out.= "<table border=\"0\" cellspacing=\"1\" cellpadding=\"2\" class=\"" .$strLineClass  . "\" width=\"220\">\n";
	$out.= "<tr>\n";
	$out.= "<td>\n";
	$out.=GenericInput(qpre . "searchterm", $strSearchTerm, false, "", "", "", "text" , 12, "", 1);
	$out.= "</td>\n";
	...
	$out.= "</td>\n";
	$out.= "<td>\n";
	$strConfig="start|start-end|end-practitioner|practitioner-type|type-notes|notes";
