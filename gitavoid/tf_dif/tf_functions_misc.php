			...
			else
			{
				...
				{	
					...
					else if($type=="file_keytorecord")
					{
						...
						$inputtag.= GenericInput($thisfieldname , $strThisDefault,  false,  "",   "",  "", "file",$size, "", $height) ;
					}
					else if($type=="date")
					{
...
function foreigntablepulldown($strDatabase, $strTable, $strIDField, $intDefault, $strLabelField="", &$namereturn, $bwlHiddenReturn=false, $strPreferredNameField="", $addedselectpairs="", $strWhereClause="", $recordforusewithwhereclause="", $truncsize=35, $strSortColumn="", $bwlSortDescending=false)
...
//THIS FUNCTION IS BEGGING TO BE REFACTORED!!!
{
	...
	{
		$fieldlist= "a." . $strIDField . ",a." . join(",a.", $arrLabelFields);
	}
	else
	{
		$fieldlist="*";
	}
	$strSQL="SELECT " . $fieldlist . " FROM " . $strDatabase . "." . $strTable . " a ";
...
function AdministerType($strDatabase, $strTable, $strAdmin, $id="", $user_id="")
//Returns an integer determining what a TF user is allowed to do.
//2 is lots of power, 0 is none.  1 means read but no write.
{
	if( $user_id!="")
	{
		$strAdmin= GetAdminName($strDatabase, $user_id);
	}
	//die("-" . $strAdmin);
	$a=GetLoginTableArray();
	$sql=conDB();
	$strSQL="SELECT permission_type_id as 'p', id_range_lowend, id_range_highend FROM  " . $strDatabase . "." . tfpre . "permission p LEFT JOIN " . $strDatabase . "." . tfpre . "admin a ON p.admin_id=a.admin_id WHERE a." .$a["username"] . " = '" . singlequoteescape($strAdmin) . "' AND p.table_name='" . $strTable . "'";
...
function GetAdminName($strDatabase, $user_id)
//Given the name of an TF user, return the user_id.  Could look in different tables depending on how
//authentication is configured in constants.php.
{
	$a=GetLoginTableArray();
	$sql=conDB();
	$strSQL="SELECT " .$a["username"] . " FROM  " . $strDatabase . "." . $a["table"] . "  WHERE " . $a["user_pk"]. " = " .intval($user_id) ;
	//echo $strSQL;
	$records = $sql->query($strSQL);
	$record=$records[0];
