...
include_once('tf_save_form.php');
include_once('tf_functions_serialize_form.php');
...
function main()
{
	...
	else
	{
	 	if ($strUser!="")
		{
			...
			else
			{
				...
				else if ($isSuperAdmin  && $strTable==""  && !$bwlHadOurContent)
				{
				
					$out.=tablebrowser($strDatabase, $strPHP, $strUser);
					$bwlHadOurContent=true;
				}
				else
				{
			...
			if (!contains($mode,"create") && (contains($mode,"view")  || (contains($mode,"save") ) || contains($mode,"delete") )    &&  !contains($strConfigBehave,"complete"))
			{
				...
				if ($intAdminType>0)
				{
					...
					if(!$bwlHadOurContent)		
					{
				 		...
				 		$strFieldConfig.="*" . tfpre . "relation|relation_id|table_name|column_name|disp_column_name|f_table_name|f_column_name|f_disp_column_name";
	...
	if (function_exists(toolnav) && !$bwlSimplifyLogin && !contains($_REQUEST[qpre . "dm"], "notopcrap"))
	{
