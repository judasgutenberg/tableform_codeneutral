...
{
	include_once('htmlconversion.php');
}
		...
		if ($intAdminType>1)
			{
				...
				if($_REQUEST[qpre . "savemap"]!="")
				{
						...
						foreach($arrPos as $thistable)
						{
							...
							if($strTable!=""  )
							{
								...
								if(!inList($tabledeletes, $strTable))
								{
									...
									if($strTable !="")
									{
										//echo $strTable . " " .  $top . " " .$left . "<BR>";
										UpdateOrInsert($strDatabase,tfpre . "dbmap_table", Array("dbmap_id"=>$dbmap_id, "table_name"=>$strTable ), Array("top_pos"=>$top, "left_pos"=>$left, "color_basis"=>$color) );
									}
									//echo mysql_error() ."-<br>";
								}
					...
					//if($bwlNewMap)
					//{
						//header("location: " . $strPHP . "?" . qpre . "db=". $strDatabase . "&dmbap_id=" . $dbmap_id_out);
					//}
...
function DBMap($strDatabase, $strPHP, $tablelist, $dbmap_id="")
{
	...
	$intPixPerChr=8;
	$intColMax=0;
	$alltables=ListTables($strDatabase);
	if( $dbmap_id=="")
	{
	...
	$out.= "<form id='idBForm' method=\"post\" name=\"BForm\" action=\"tf_db_map.php\" onsubmit='return(MapSave())'>\n";
	...
	foreach($arrSort as  $item)
	{
			...
			{ 
				if(contains($arrDif[$i], " "))
				{
					...
					if($testDif>$intCols +2  && !$bwlLocated  && $thiscollen>=$intColLength)
					{
						...
						$out.="\n<div id=\"iddiv-" .  $strTable . "\" style=\"overflow:auto;position: absolute;top:" .$thisy . "px;left: " . $thisx . "px;\">" . $thistableHTML . "</div>";
						$z++;
						$bwlLocated=true;
		...
		else   
		{
			...
			$out.="\n<div id=\"iddiv-" .  $strTable . "\" style=\"overflow:auto;position: absolute;top:" . $y . "px;left: " . $x . "px;\">" . $thistableHTML . "</div>";
			if($intDif>0)
			{
	...
	$out.="<script type=\"text/javascript\">";
	$out.= "var unusedtables='" . $unusedtables . "'\n";
	$out.="</script>";
	$out.="<div>  map name: ";
	...
	$out.= " | <a href=\"javascript:ColorPicker('localselect(document.BForm.search','recolorfound()')\">color code tables matching a search</a>";
	$out.= " | <a href=\"" . $strPHP . "?" . qpre . "db=" .$strDatabase . "&" . qpre . "mode=exportpdf&dbmap_id=" . $dbmap_id . "\">export PDF</a></div>";
	$out.= HiddenInputs(array("db"=>$strDatabase, "tablelist"=> $tablelist, "positions"=> "", "newtablename"=>"", "taskstodo"=>""));
...
function RelationDump($strDatabase, $tablelist, $bwlRemoveDups=true)
//Creates a space and pipe delimited string containing the essence of the tf_relation's FK data
//This is used by the javascript in tf_drawrelationships.js to draw all the relationships between tables.
//This all has to be in javascript to allow the user to move things around and such.
//2010-08-05: i added code to remove dups that can accumulate in the relation table. this is a good place for that
//code because it provides a place to comprehensively analyze and clean up all relation data
//and the problems with dups are most evident in the database map
{
	$sql=conDB();
	$out="";
	...
	foreach($records as $record)
	{
		...
		if(inList($tablelist, $strTable) &&  inList($tablelist, $strFKTable)  || $tablelist=="")
		{
			...
			$strThisDelimitedRecord=$strTable . " " . $strColumn . " " . $strFKTable . " " . $strFKColumn  . "|";
			$bwlCanAdd=true;
			if($bwlRemoveDups)
			{
				//inList($strList, $item, $MatchOnlyType="", $strDelimiter=" ") 
				if(inList($out, $strThisDelimitedRecord, "" , "|"))
				{
					$bwlCanAdd=false;
					//now we need to delete the dup record in the relation table!
					DeleteBySpec($strDatabase, tfpre . "relation", Array("relation_id"=>$relation_id));
				}
			}
			if($bwlCanAdd)
			{
				$out.=$strThisDelimitedRecord;
...
//Creates a space and pipe delimited string containing position info for a dbMap
{
	...
	foreach($records as $record)
	{
		$strTable=$record["table_name"];
		$top=$record["top_pos"];
		$left=$record["left_pos"];
		$color=$record["color_basis"];
		$out.=$strTable . " " . $top . " " . $left . " " . $color . "|";
...
function MapForm($strDatabase, $dbmap_id)
{
	...
	$preout.= "<form id='idBForm' method=\"post\" name=\"BForm\" action=\"tf_db_map.php\">\n";
	...
	foreach ( $tables as  $tablename )
	{
		...
		$checkcell.="<a href=\"javascript:alldumpcheckboxes('map[]', 'revbefore', '', '','" . $tablename . "' )\">^</a> ";
		$checkcell.="<a href=\"javascript:alldumpcheckboxes('map[]', 'revafter', '' , '', '" . $tablename . "' )\">v</a>";
		$out.=htmlrow($strThisBgClass, 
			$tablename, 
			$count,
			$fieldcount,
