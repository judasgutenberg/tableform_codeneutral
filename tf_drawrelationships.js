...
var globallocalcolor;
var relationshipsexist=false;
var globalxofaction=0;
var globalyofaction=0;
var globalnewtable="";
...
function MapSave()
{
	...
	for(i=0; i<arrDivs.length; i++)
	{
		...
		if (thisid.indexOf("iddiv")>-1)
		{
			...
			var color=thisdiv.style.backgroundColor;
			if(color!="")
			{
...
{
	//http://boostup.largeanimal.com/tf/tf_dbtools.php?x_db=BOOSTUP&x_table=mysql_errorlog&x_mode=empty
	ajax.location.href=tfpre + "dbtools.php?x_db=" + database + "&" + qpre + "table=" + tablename + "&" + qpre + "mode=empty";
}

function DataScript(tablename)
{
 
	//var arrAnchortext= new Array('<br/><br/>close');
	//var arrLink= new Array('javascript:thisdiv=parent.document.getElementById(\"ajaxback\");thisParent=GetParent(thisdiv);thisParent.removeChild(thisdiv);');
	//PopupMenuAtMousePosition(arrAnchortext, arrLink, "ajaxback", "onmouseup",900);
	remote = window.open(tfpre + "scripttasks.php?x_db=" + database + "&" + qpre + "table=" + tablename + "&" + qpre + "mode=sqldata&" + qpre + "displaymode=text","copypaster","menubar=yes,height=600,width=800,scrollbars=yes");
	remote.focus();
	//ajax.location.href=tfpre + "scripttasks.php?x_db=" + database + "&" + qpre + "table=" + tablename + "&" + qpre + "mode=sqldata";
}

function CreateScript(tablename)
{
	//var arrAnchortext= new Array('<br/><br/>close');
	//var arrLink= new Array('javascript:thisdiv=parent.document.getElementById(\"ajaxback\");thisParent=GetParent(thisdiv);thisParent.removeChild(thisdiv);');
	//PopupMenuAtMousePosition(arrAnchortext, arrLink, "ajaxback", "onmouseup", 400);
	remote = window.open(tfpre + "scripttasks.php?x_db=" + database + "&" + qpre + "table=" + tablename + "&" + qpre + "mode=sqlcreate&" + qpre + "displaymode=text","copypaster","menubar=yes,height=600,width=800,scrollbars=yes");
	remote.focus();
	//ajax.location.href=tfpre + "scripttasks.php?x_db=" + database + "&" + qpre + "table=" + tablename + "&" + qpre + "mode=sqlcreate";
}
...
function AddTableToPositionData(tablein,x,y)
{
	RemoveTableFromPositionData(tablein);
	positiondata=positiondata + "|" + tablein + " " + x + " "  + y;
	
	//also, cut this table from unusedtables, which is a global
	var arrUnused=unusedtables.split(" ");
	unusedtables="";
	for(i=0; i<arrUnused.length; i++)
	{
		if(arrUnused[i]!=tablein)
		{
			unusedtables+=" " + arrUnused[i];
		}
	}
 	unusedtables=unusedtables.substr(1);
}
...
function mrstart(id)
{ 
 	document.onmouseup=  new Function ("return"); 
...
{ 	
 	...
 	//alert(globalBwlMakingRelation + "\n%" + id + "\n=" + globalidstart + "\n+" + globalpopupmode + "\n*" + globalidforpopup +"\n:" + globalidhovering );
	if(typeof(relationshipdata)=="undefined")
	{
		return;
	}
	if(globalidstart==""  && id!='x')
	{
		//what i'd use for a context-sensitive add-a-table popup
		var thisdiv=document.getElementById('idnotable');
		var thisdiv2=document.getElementById('idmapdialog');
		if(!thisdiv  && !thisdiv2)
		{
	
			return NoTablePopup();//create idnotable
			
		}
		//i'd like to make it so the idnotable goes away when i click somewhere other than on its grey patch 
	...
	if(!id && globalpopupmode=="")
	{
		if(thisitem)
		{
			removeelement("idmapdialog");
			removeelement("idnotable");
			removeelement("idblankzone");
			globalpopupmode="";
		}
	}
	...
	//globalidforpopup="";
	setTimeout('document.onmouseup=  new Function (\"return mrdone(0)\");',50);
	//document.onmouseup=  new Function ("return mrdone('0')"); 
}
...
{ 
	...
	document.body.style.cursor='pointer';
	var thisitem=document.getElementById("idnotable");
	if(thisitem)
	{
 
		removeelement(thisitem);
	}
...
{
	//globalidstart=4; //<<that was a hack. but it worked!
	removeelement('idblankzone');
	//alert(tablename);
	remote = window.open(tfpre + "scripttasks.php?x_db=" + database + "&" + qpre + "table=" + tablename + "&" + qpre + "mode=tableinfo&" + qpre + "displaymode=ajax&" + qpre + "ajaxback=idBForm&" + qpre + "windowback=parent&" + qpre + "x=" + globalxofaction + "&" + qpre + "y=" + globalyofaction + "&" + qpre + "z=" + globalz + "","ajax","menubar=yes,height=600,width=800,scrollbars=yes");
	globalz++;
	removeelement('idnotable');
 	removeelement('idblankzone');
 
	AddTableToPositionData(tablename,globalx,globaly);

	setTimeout("ReDragInitTables();  ",1600);
	//Drag.init(document.getElementById('iddrag-' + tablename));
...
function ReDragInitTables()
{
	...
	for(i=0; i<arrDivs.length; i++)
	{
		...
		var thisinfo=arrDivs[i].split(" ");
		var tablename=thisinfo[0];
		var objtoinit=document.getElementById('iddrag-' + tablename);
		if(objtoinit)
		{
...
function MissingTablePicker()
{
	globalxofaction=globalx;
	globalyofaction=globaly;
	var i;
	//alert(unusedtables);
	
	var theseunusedtables="<b>Add&nbsp;an&nbsp;Existing&nbsp;Table</b> " + unusedtables;
	var arrUnused=theseunusedtables.split(' ');
	var arrLink = new Array();

	for(i=0; i<arrUnused.length; i++)
	{
		if(arrUnused[i].indexOf("&")>0)
		{
			arrLink[i]="";
		}
		else
		{
			arrLink[i]="javascript:addtable('" + arrUnused[i] + "')";
		}
	}
	arrAnchortext=arrUnused;
	//arrLink=arrUnused;
	//var arrLink= new Array
		//(
		//'javascript:addtable()'
	...
	removeelement('idblankzone');
	PopupMenuAtMousePosition(arrAnchortext, arrLink, "idnotable", "onmouseup", 240);
	removeelement('idmapdialog');
	

}

function NoTablePopup()
{
	var i;
	//alert(unusedtables);
	if(globaly>50)//only do this if we are 50 pixels from the top 
	{
	 
		var arrLink = new Array("javascript:MissingTablePicker()","javascript:NewTableCreator()" );
 
		arrAnchortext=new Array("Add Existing Table Here", "Create New Table Here");
		...
		PopupMenuAtMousePosition(arrAnchortext, arrLink, "idblankzone", "onmouseup", 150);
		removeelement('idmapdialog');
	}
}

function NewTableCreator()
{
	//alert("Feature not yet implemented.");
	//close caller:
	globalxofaction=globalx;
	globalyofaction=globaly;
	//globalidstart=4;//hackyhack
 
	removeelement('idblankzone');
	var ajaxurl=tfpre + "scripttasks.php?x_db=" + database + "&" + qpre + "table=<table/>&" + qpre + "mode=tableinfo&" + qpre + "displaymode=ajax&" + qpre + "ajaxback=idBForm&" + qpre + "windowback=parent&" + qpre + "x=" + globalxofaction + "&" + qpre + "y=" + globalyofaction + "&" + qpre + "z=" + globalz + "";

	editselected("", tfpre + "table_maker.php?" + qpre + "table=&" + qpre + "db=" + database + "&" + qpre + "forkurl=" + escape(ajaxurl) + "&" + qpre + "forktarget=" + "opener.ajax.location.href" + "&" + qpre + "postforkjstasks=" + "opener.document.BForm['" + qpre + "newtablename'].value='<table/>';window.close();");
  

	//have to add the table to the list of tables, etc.
...
function popuptabledialog(id)
{
	...
	var arrAnchortext= new Array('edit definition', 'view data', 'remove table from map', 'empty table', 'create script', 'data script','change color');
	var arrLink= new Array
		(
		'javascript:edittable("' + id + '","schema")',
		'javascript:edittable("' + id + '","data")',
		'javascript:edittable("' + id + '","remove")',
		'javascript:edittable("' + id + '","emptytable")',
		'javascript:edittable("' + id + '","createscript")',
		'javascript:edittable("' + id + '","datascript")',
		'javascript:edittable("' + id + '","changecolor")'
		)
		
	PopupMenuAtMousePosition(arrAnchortext, arrLink, "idmapdialog", "onmouseup", 150);
	removeelement('idnotable');
 	removeelement('idblankzone');
...
function popupcolumndialog(id)
{
	...
	{ 
		...
		if(arrFound)
		{
	 		// PopupMenuAtPosition(arrAnchortext, arrLink, id, x, y, width, color, actiontype)
			PopupMenuAtMousePosition(arrAnchortext, arrLink, "idmapdialog", "onmouseup", 120);
		}
		removeelement('idblankzone');
...
function PopupMenuAtMousePosition(arrAnchortext, arrLink, id, actiontype, width)
{
	//alert(id);
	...
	PopupMenuAtPosition(arrAnchortext, arrLink, id, xtouse, globaly, width,  "cccccc", actiontype)
}
...
function editrelation(id)
{
	...
	if(thisrid=="")
	{
		...
		if(thisMatch)
		{
			editselected("", "tf.php?" + qpre + "dm=notopcrap" + "&" +qpre + "mode=edit&" + qpre + "table=" + tfpre + "relation&" +qpre + "idfield=relation_id&table_name=" + thisMatch[0] + "&column_name=" + thisMatch[1] + "&f_table_name=" + thisMatch[2] + "&f_column_name=" + thisMatch[3] + "&relation_type_id=0&" + qpre + "db=" + database);
		}
	}
	else
	{
	
	editselected("", "tf.php?"+ qpre + "dm=notopcrap" + "&" + qpre + "mode=edit&" + qpre + "table=" + tfpre + "relation&" +qpre + "idfield=relation_id&relation_id=" + thisrid + "&" + qpre + "db=" + database);
	}
}
...
function edittable(id, thistype)
{
	...
	if(thistype=="data")
	{
		editselected("", "tf.php?" + qpre + "dm=notopcrap" + "&" + qpre + "mode=view&" + qpre + "table=" + thistable + "&" + qpre + "db=" + database)
	}
	...
	else if(thistype=="createscript")
	{
		CreateScript(tablename);
	}
	else if(thistype=="datascript")
	{
		DataScript(tablename);
	}
	else if(thistype=="emptytable")
	{
		if(confirm("Are you certain you want to empty the table named " + tablename + "?"))
		{
	...
	else //thistype=='schema'
	{
		var ajaxurl=tfpre + "scripttasks.php?x_db=" + database + "&" + qpre + "table=<table/>&" + qpre + "mode=tableinfo&" + qpre + "displaymode=ajax&" + qpre + "ajaxback=idBForm&" + qpre + "windowback=parent&" + qpre + "x=" + globalxofaction + "&" + qpre + "y=" + globalyofaction + "&" + qpre + "z=" + globalz + "";
		
		editselected("", tfpre + "table_maker.php?" + qpre + "table=" + thistable + "&" + qpre + "db=" + database + "&" + qpre + "forkurl=" + escape(ajaxurl) + "&" + qpre + "forktarget=" + "opener.ajax.location.href" + "&" + qpre + "postforkjstasks=" + "opener.document.BForm['" + qpre + "newtablename'].value='<table/>';window.close();");
...
function ChangeTableColor(winobj, tablename, newcolorbasis)
{
	...
	if(thisdiv)
	{
		thisdiv.style.backgroundColor='#' + newcolorbasis;
...
function recolorfound()
{
	...
	var newcolor=globallocalcolor;
	if(!newcolor)
	{
		//alert("You must select a color.");
		//using PopupMenu as a non-modal alert
		PopupMenuAtMousePosition(Array("You must select a color."), Array(""), "idnonmodalerror", "onmouseup", 240);
		setTimeout("removeelement('idnonmodalerror')", 500);
	}
	else
	{
...
function localselect(objectname, color)
{
	objectname.style.backgroundColor='#' + color;
	globallocalcolor=color;
}

function situationscan()
{
 	if(document.BForm)
	{	
		if(document.BForm[  qpre + 'newtablename'])
		{
			if(document.BForm[  qpre + 'newtablename'].value!=globalnewtable)
			{
				globalnewtable=document.BForm[ qpre + 'newtablename'].value;
				AddTableToPositionData(globalnewtable,globalxofaction,globalyofaction);
				removeelement('idblankzone');
				taskstodo=document.BForm[ qpre + 'taskstodo'].value;
				if(taskstodo!="")
				{	
					document.BForm[ qpre + 'taskstodo'].value="";
					eval(taskstodo);
				}
				//have to do this to make everything draggable again:
				setTimeout("ReDragInitTables();  ",1200);
			}
		}
	}
	setTimeout("situationscan()", 200);
}
...
if(typeof(positiondata)!= "undefined")
{
	...
	for(i=0; i<arrDivs.length; i++)
	{
		...
		var top=thisinfo[1];
		var left=thisinfo[2];
		var color=thisinfo[3];
		//alert('iddiv-' + tablename  + ":" +left );
		if(tablename!="")
		{
			//alert("iddiv-" + tablename + "-") ;
			if(color!="")
			{
				ChangeTableColor(window, tablename, color);
			}
