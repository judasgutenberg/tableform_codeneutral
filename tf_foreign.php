			...
			if ($intAdminType>1)
				{
					...
					$out.= ForeignKeyReferralLists($strDatabase, $strTable,$idfield , $id, "tf.php", $strTheirTable, $rec, 500, $strDisplayMode, $strForeignLimit);
				}
		}
...
//inside a particular database, find all the tables that have a foreign key relationship with the given item and list them in grouped clusters, complete with various editing options
	...
	if ($strOurID!="")
	{
		...
		{
			$strTheirNameColumn="";//for some reason i'd stopped doing this!!
			$strTheirTable=$fkrecord["table_name"];
			
			$complete_list_data=$fkrecord["complete_list_data"];
			$tool_guidance=$fkrecord["tool_guidance_id"];
			$relation_type_id=$fkrecord["relation_type_id"];
			$disp_column_name=$fkrecord["disp_column_name"];
			$sort_column_name=$fkrecord["sort_column_name"];
			$sort_descending=$fkrecord["sort_descending"];
			$f_disp_column_name=$fkrecord["f_disp_column_name"];
			$f_sort_column_name=$fkrecord["f_sort_column_name"];
			$f_sort_descending=$fkrecord["f_sort_descending"];
			
			//echo $tool_guidance . " "  .  $relation_type_id . " " . $strTheirTable . "<BR>";
			//echo ($tool_guidance==1 && $relation_type_id==0) . "<BR>";
			if(!($tool_guidance==1 && $relation_type_id==0))
			{
				if($strForeignLimit=="" || $strForeignLimit!=""  && inList($strForeignLimit,  $strTheirTable)) //limit associated items to those indicated by restriction if there is one
				{
					...
					$thistableinfo="";
		 			if(function_exists("associated_list_" . $strTheirTable))
					{
						//echo  $strTheirTable . "<BR>";
					 
						$out.=  "<tr><td>" . call_user_func("associated_list_" . $strTheirTable, $fkrecord["column_name"], $strOurDataToUse, $strOurTable) . "</td></tr>";
						//die($thistableinfo);
					}
					else
					{
					//echo $strTheirTable . "<br>";
					//hmm this might work for some cases, but it definitely screws up some too:
					//this would seem to do precisely the wrong thing:
					//$strTheirNameColumn=$fkrecord["display_column_name"]; 
					//echo $strTheirNameColumn . "<BR>";
					...
					//echo $intCount . "<br>";
					//echo  $strTheirTable . " "  . $strTheirNameColumn .  "<BR>";;
					...
					$strSortColumn="";
					$sorttools="";
					$strSortColumn=gracefuldecay($sort_column_name,SortColumn($strDatabase, $strTheirTable));
					//echo "!" . $strSortColumn;
					if ($strSortColumn!="")
					{
					...
					// echo $strTheirNameColumn . "<BR>";
						if ($strTheirNameColumn==""  || $bwlForceMap)//then we have a mapping table on our hands!
						{
							...
							if ($intFKcount>1)//if a mapping table has two or more foreign keys beyond
															...
															//pure mapping table
															//this definitely causes suboptimal results
															//on featurewell's clickthrough table, which is a mapping table.
							{
						...
						foreach ($records as $record )
						{
							...
							if ($strMappedTable!="") //in the case where we are looking at a mapped table
							{
								...
								//echo $fkrecord["table_name"] . " " . $strSQL . "<br>";
							...
							if ($strNameToUse=="")//in which i handle the case of complex mapping tables
							{
								...
								for ($intFKCounter=0; $intFKCounter<$intFKcount+1; $intFKCounter++)
								{
									...
									//echo $strNameFieldFromRelationTable;
									...
									if($record[$arrFKInfo[$intFKCounter][0]]!="")
									{
		...
		if(function_exists("associated_list"))
		{
		 
			$out.=  "<tr><td>" . call_user_func("associated_list",  $strIDField, $strOurID,$strOurTable) . "</td></tr>";
		}
		if ($out!="")
		{
