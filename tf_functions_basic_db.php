...
class sqldb //I'm not big into PHP classes, but I use one here as part of the legacy of how this tool came to be.
{
	...
	function query($strSQL , $type="gusstyle", $strDatabase="")
	{
		...
		GLOBAL $db_type;
		$bwlUpdateCache=false;
	 
		if(defined("cachequerytablelist")  && $_REQUEST[qpre . "cache"]!="clear")
		{
			if(!beginswith(trim($strSQL), "UPDATE")  && !beginswith(trim($strSQL), "INSERT")) //don't cache db change recordsets
			{
				$arrQueryTableList=explode(" ", cachequerytablelist);
				foreach($arrQueryTableList as $thistablepair)
				{
					$arrCacheinfo=explode(".", $thistablepair);
					if(contains($strSQL, $arrCacheinfo[0]))
					{	
						$recordcacheroot=recordcache;
						if(file_exists($recordcacheroot))
						{
							$strRecordPath=$recordcacheroot . "/" . md5($strSQL) . ".txt";
							if(file_exists($strRecordPath))
							{
								if(time()-filemtime($strRecordPath)>$arrCacheinfo[1])
								{
								...
								{
									//return old one
									$handle=fopen ($strRecordPath, "r");
									$content=fread($handle, filesize($strRecordPath));
									fclose($handle);
		...
		if($bwlUpdateCache &&  ($out)!="")
		{
			if(file_exists($recordcacheroot))
			{
			    $handle = fopen($strRecordPath, "w");
	       		if (!fwrite($handle, serialize($out)))
				{
					//echo "caching failed!";
				}
				$handle = fopen($strRecordPath. ".sql", "w");
				if (!fwrite($handle,  ($strSQL . "\n\n" . serialize(debug_backtrace()))))
				{
					//echo "caching failed!";
				}
			}
...
function getEngine($strDatabase, $strTable)
{
    $sql=conDB();
    $strSQL="SELECT ENGINE FROM information_schema.TABLES WHERE TABLE_SCHEMA='" . $strDatabase . "' AND TABLE_NAME='" .$strTable . "'";
    //echo $strSQL . "<BR>";
...
function TableExists($strDatabase, $strTable)  //need to update for addDatabaseIfNecessary($strDatabase, $strTable)
...
//As of Dec 18th, 2007, accepts a $strNameColumn in cases where this is known
{
	...
	$sql=conDB();
	$strSQL="SELECT * FROM " . addDatabaseIfNecessary($strDatabase, $strTable)  . " WHERE " .  $strIDField . " = " . $strID;
...
//modified 10-2-2007 to allow for the returning of a whole row
{
	...
	$strSQL="SELECT " . $fieldtosearchfor . " FROM " . addDatabaseIfNecessary($strDB, $strTable)  . " WHERE " . $strWhereClause;
	//echo $strSQL . "<BR>";
	if($bwlDebug)
	{
		echo $strSQL . "<br>";
...
function addDatabaseIfNecessary($strDatabase, $strTable)
{
	if(contains($strTable, ".")  || trim($strDatabase)=="")
	{
		return $strTable;
	}
	return $strDatabase . "." . $strTable;
}
...
//explains the columns across several tables linked by relations in the relation table or do a simple explain
{
	...
	$records = $sql->query("EXPLAIN " . addDatabaseIfNecessary($strDatabase, $strTable)  );
...
function removeTics($in)
{
	...
	else //for odbc-style tics
	{
		$out=RemoveLastCharacterIfMatch($in, "]");
...
//judas gutenberg may 25 2010
	...
	if(beginswith(strtolower($out), "and"))
	{
		$out=substr($out, 3);
...
function removeslashesifnecessary($strIn)
{
	//echo get_magic_quotes_gpc() . "<BR>";
	//echo get_magic_quotes_gpc() . "==getmagicbullshit";
	...
	if(!is_array($strIn))
	{
		...
		{
			//echo addslashes($strIn) . "===<BR>";
		   return stripslashes($strIn);
		}
		else
		{
			//echo addslashes($strIn) . "++++<BR>";
...
function ourDBescape($strIn, $type="")
{
	if($type=="odbc")
	{
...
//this function has pretty much replaced my use of explicit SQL for updates and inserts, though Saveform still does things that way
{
	if(1==2)
	{
		...
		echo $strDatabase . "<P>";
		var_dump($arrDescribedValuePairs);
		echo "</DIV>";
		echo "<DIV style='background-color:ffccff'>";
		echo $strDatabase . "<P>";
		var_dump($arrAlteredValuePairs);
		echo "</DIV>";
		die();
	}
	GLOBAL $db_type;
	...
	$strSQL="SELECT * FROM " . addDatabaseIfNecessary($strDatabase, $strTable) . " WHERE ";
	//echo $strSQL . "<p>";
	$strAlterSQL="";
	$strUpdateSQL="";
	$strAlterSetSQL="";
	...
	if(is_array($arrDescribedValuePairs))
	{
		...
		foreach($arrDescribedValuePairs as $k=>$v)
		{
			...
			$k=ensureEnclosure($k, $startEnclosure, $endEnclosure);
			...
			if(array_key_exists(removeTics($k), $arrKnownFields))
			{
				...
				if($bwlEscape)
				{
		 			$valtostore= ourDBescape($v, $db_type);
				}
	...
	$strAlterWhereSQL= substr($strAlterWhereSQL, "0", strlen($strAlterWhereSQL)-4); 
 	$strDescWhereSQL= substr($strDescWhereSQL, "0", strlen($strDescWhereSQL)-4); 
	...
	if(endswith($strSQL, "WHERE "))
	{
		$strSQL.=$strDescWhereSQL;
	}
	...
	//echo count($records) ." " .   $strTable . "_<br>";
	...
	{
		//need an update
		$strSQL="UPDATE " . addDatabaseIfNecessary($strDatabase, $strTable)   . " SET ". $strAlterSetSQL . " WHERE " . $strAlterWhereSQL;
		 
	}
	else if (count($records)<1  ||  !is_array($arrDescribedValuePairs)) //this code won't allow a new record that is identical except for pk. may need to think about it some more
	{
		//echo $strTable;
		//need an insert
		...
		$columnlist=JoinList(",", $strColumnList, $strColumnListDesc);
		$valuelist=JoinList(",",$strValueList ,$strValueListDesc);
		 
		//echo $columnlist . "=columnlistone<BR>";
		//echo $valuelist . "=columnlis2<BR>";
		//this is a wasteful procedure -- ideally i'd clean up my items enough not to have to do this:
		//EliminateItemsInListsThatHaveDupNamesInListOne(",", $columnlist, $valuelist);
		//echo $columnlist . "=columnlistoneafter<BR>";
		//echo $valuelist . "=columnlis2after<BR>";

		if(count($valuelist)!="")//don't insert empty rows!
		{
			//echo "$";
			$strSQL="INSERT INTO " . addDatabaseIfNecessary($strDatabase, $strTable) . "(" . $columnlist . ") VALUES(" . $valuelist  . ")";
			//echo $strSQL . "<BR>";
		}
	...
	if($strSQL!="")
	{
		...
		//echo $strSQL . "=<br>" . mysql_error() . "<br><br>";
		//die( $strSQL );
		...
		//	if($strDatabase=="dbo")
			{
				//echo "<div style='background-color:#ffcccc'>" .  $strSQL  . "<br>" .  mysql_error() . "^^<BR></div>";
			}
	...
	{
		//echo $strDatabase . "__" .   $strTable  . "##";
		return "You cannot insert a functionally-duplicate record.";
	}
	//echo $idInsert . "=<br>";
	$out= gracefuldecay(sql_error(),$idInsert) ;
	//echo "-" . function_exists("ForkInsert");
...
function TableList($strDatabase, $likeclause="")  //might have to update this for addDatabaseIfNecessary($strDatabase, $strTable)
{
	GLOBAL $db_type;
	if( $db_type=="odbc")
	{
...
//Looks up the multi-table info given a column
{
		...
		foreach ($records as $record)
		{
			if($mode=="")
			{
				...
				return array( $record["f_table_name"],  $record["f_column_name"], $record["table_name"], $record["column_name"], $record["f_disp_column_name"], $record["tool_guidance_id"], $record["narrowing_conditions"], $record["relation_id"], $record["max_dropdown"], $record["f_sort_column_name"], $record["f_sort_descending"], $record["disp_column_name"], $record["sort_column_name"],  $record["disp_column_name"], $record["complete_list_data"]) ;
			}
...
//modified for MySQL 2-7-06
{
	...
	foreach ($records as $record)
	{
		if ( $record["f_table"] != $strMappedTableToAvoid  )
		{
			...
			if ($n==$right)
			{
				return array(  $record["column_name"], $record["f_table_name"],  $record["f_column_name"],  $record["f_disp_column_name"]) ;
			}
...
function NextPrimaryKey($strDatabase, $strTable)//might need to update this for addDatabaseIfNecessary($strDatabase, $strTable)
{
	//this function is not a wise way to predict future PKs in a hectic multiuser environment
	$sql=conDB();
...
//modified sept 2 2008 to deal with compound pks, returning null in that case
{
	...
	foreach ($records as $k => $v )
	{
		if ($v["Key"]=="PRI")
		{
			...
			$strSQL="SELECT MAX(" . $v["Field"]. ") FROM " . addDatabaseIfNecessary($strDatabase, $strTable);
...
{
	$sql=conDB();
	$strSQL="SELECT " .  $strFieldName . " FROM " . addDatabaseIfNecessary($strDatabase, $strTable) . " WHERE " . $strFieldName . "='" . $ID . "'";
	$records = $sql->query($strSQL);
 	if (count($records)>0)
	{
		return true;
...
function countrecords($strDatabase , $strTable, $strWhereClause="", $recordforusewithwhereclause="")
//Returns the number of records in a table.
{
	$sql=conDB();
	$strSQL="SELECT COUNT(*) FROM " . addDatabaseIfNecessary($strDatabase, $strTable) . " " .  MergeSQLConditionals(MergeLimitingConditionsWithRSData($strWhereClause, $recordforusewithwhereclause) , "");
	//echo $strSQL . "<BR>";
...
{
	$sql=conDB();
	$strSQL="SELECT * FROM " . addDatabaseIfNecessary($strDatabase, $strTable) . " WHERE " .  $fkname ."='" . singlequoteescape($fk) . "'";
...
function TypeSQL($strType, $strSize="", $strAfterComma="")
{
	...
	if (!inList("mediumtext time longtext datetime text date timestamp bit blob", $strType))
	{
		$out= $strType . "(" . $strSize . ") ";
	}
	else
...
//can make an update statement if $arrSpec is given
{
	...
	if(is_array($arrSpec))
	{
		$out="UPDATE " . addDatabaseIfNecessary($strDatabase, $strTable) ;
	}
	else
	{
		$out="INSERT INTO " . addDatabaseIfNecessary($strDatabase, $strTable);
	}
	$nameclause="";
	$dataclause="";
	foreach($arrIn as $key=>$value)
	...
	if($strLimitingConditions!=""  && $rsContext!=""  && !$bwlSimpleLimit)
	{
		if(beginswith(strtolower($strLimitingConditions), "join")  || beginswith(strtolower($strLimitingConditions), "left join")  || beginswith(strtolower($strLimitingConditions), "where"))
		{
			if(is_array($rsContext))
			{
				foreach($rsContext as $k=>$v)
				{
					...
					//echo   $k . " "  . "<font color=red>" .  $strLimitingConditions . "</font><BR>";
					$strLimitingConditions=str_replace("$" . $k, $v, $strLimitingConditions);
					//echo "<font color=green>" .  $strLimitingConditions . "</font><BR>";
				}
			}
		}
		else
		{
			if(is_array($rsContext))
			{
				foreach($rsContext as $k=>$v)
				{
					//echo   $k . " "  . "<font color=orange>" .  $strLimitingConditions . "</font><BR>";
					//looks for tokens in the whereclause of the form $fieldname, and replaces them with values if they are found.
					$strLimitingConditions=str_replace("$" . $k, $v, $strLimitingConditions);
				}
			}
...
{
	$records=TableExplain($strDatabase, $strTable);
	//echo count($records) .  $strDatabase ." " .  $strTable  . "<BR>";
	if($bwlFormat=="odbc")
	{
		$strQS="[";
		$strQE="]";
...
//judas gutenberg cinquo de mayo 2010
{
	...
	foreach($records as $record)
	{
		...
		//echo $strThisPKValue . "-<BR>";
		$bwlChecked= ($return!=""  && $strThisPKValue!="");
...
function DeleteBySpec($strDatabase, $strTable, $arrSpec, $bwlDontFork=false)
{
	$sql=conDB();
	$strWhereClause=ArrayToWhereClause($arrSpec);
	$strDeleteSQL="DELETE FROM " . addDatabaseIfNecessary($strDatabase, $strTable). " WHERE " . $strWhereClause;
	//echo $strDeleteSQL . "<BR>";
	$sql->query($strDeleteSQL);
	if(function_exists("ForkDelete")  && !$bwlDontFork)
	{
		ForkDelete($strDatabase, $strTable, $arrSpec);
	}
}

function ResetAutoIncrement($strDatabase, $strTable)
{
	
	$intCalcNumber=highestprimarykey($strDatabase, $strTable);
	if($intCalcNumber=="")
	{
		return "You cannot change auto increment for the table " . $strTable . ".";
	}
	else
	{
		$sql=conDB();
		$strSQL="ALTER TABLE " . $strDatabase . "." .  $strTable . " AUTO_INCREMENT=" . $intCalcNumber;
		$sql->query($strSQL);
		return sql_error() . "\nAuto increment for the table " . $strTable . " was set to " .  $intCalcNumber . ".";
	}
}


function EliminateItemsInListsThatHaveDupNamesInListOne( $delimiter, &$listone, &$listtwo)
{
	 
	//BetterExplode($delimiter, $data,  $quotechar="\" '", $bwlLeaveQuotesInPlace=false, $strEscapeStyle="csv", $bwlDelmiterCaseInsensitive=true, $delimiterprefixnixlist="", $chrQuoteOverwrite="", $chrPHPCommentOverwrite="", $chrNonQuoteOverwrite="", $chrEndQuoteToStripFromData="")
	echo  "<div style='background-color:#ccffcc'>" .  $listone . "</div>";
	echo "<div style='background-color:#99cc99'>" .  $listtwo. "</div>";
	$arrListOne=BetterExplode($delimiter, $listone,"39", true );
	$arrListTwo=BetterExplode($delimiter, $listtwo,"39", true);
	$items="";
	$listoneout="";
	$listtwoout="";
	$outcount=0;
	foreach($arrListOne as $thisitem)
	{
		if(!inList($items, $thisitem))
		{
			$listoneout.=$delimiter . $thisitem;
			$listtwoout.=$delimiter. $arrListTwo[$outcount];
			$items.=" " . $thisitem;
		
		}
		$outcount++;
	}
	$listone=RemoveFirstCharacterIfMatch($listoneout, ",");
	$listtwo=RemoveFirstCharacterIfMatch($listtwoout, ",");
	
	echo "<div style='background-color:#ffffcc'>" . $listone . "</div>";
	echo  "<div style='background-color:#cccc99'>" . $listtwo. "</div>";
}	
