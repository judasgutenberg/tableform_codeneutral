...
//creates a new relation if one matching the spec doesn't exist
	...
	if(count($tables)<1)
	{
		...
		$error=sql_error();
                $out.=IFAThenB($error, "<hr>" . $strSQL . "<br>") . $error;		
	}
	else
	{
		//$out.= "Needed relation already exists:" . $linkertable . "." .$linkerfield . "->" . $linkeetable . "." . $linkeefield . ":" . $type . "<br>";
	}
        //echo getEngine($strDatabase, $linkeetable) . "<BR>";
        if(strtolower(getEngine($strDatabase, $linkertable))=="innodb" && strtolower(getEngine($strDatabase, $linkeetable))=="innodb" )
        {
            //echo "D";
            //might as well tell INNODB about the relation as well if the tables are both INNODB
            $fkName="fk_" . $linkertable . "$" . $linkerfield. "$" .  $linkeetable . "$" . $linkeefield;
           // $strSQL="ALTER TABLE " . $strDatabase . "."  .  $linkertable . " DROP FOREIGN KEY " . $fkName;
           // $sql->query($strSQL);
            
            $strSQL="SELECT * from information_schema.table_constraints where TABLE_SCHEMA='" . $strDatabase."' and TABLE_NAME='" .$linkertable . "' CONSTRAINT_TYPE='FOREIGN KEY'";
            $existrecords=$sql->query($strSQL);
             //echo "<br>"  . $linkeetable . ":<font color=blue>" . PKLookup($strDatabase, $linkeetable). "</font>" . intval(count($existrecords)) . "<BR>";
            if(count($existrecords)<1)
            {
                //have to make PKs real to get the next part to work
                
                if(PKLookup($strDatabase, $linkeetable)=="")
                {
                    $strSQL="ALTER TABLE " . $strDatabase . ".`"  .  $linkeetable . "` ADD PRIMARY KEY(`" . $linkeefield . "`);";
                   // echo "<font color=red>" . $strSQL . "</font><BR>";
                    $sql->query($strSQL);
                    $error=sql_error();
                    $out.=IFAThenB($error, "\n<hr>" . $strSQL . "\n<br>") . $error;	
                    
                }
                $strSQL="ALTER TABLE " . $strDatabase . ".`"  .  $linkertable . "` ADD CONSTRAINT " . $fkName  . " FOREIGN KEY  (" . $linkerfield . ") REFERENCES `" . $linkeetable . "`(" . $linkeefield . ");";
               //echo $strSQL . "<br>";
                //$sql->query($strSQL);
                $error=sql_error();
                $out.=IFAThenB($error, "\n<hr>" . $strSQL . "\n<br>") . $error;	
