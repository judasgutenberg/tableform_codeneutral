...
{
	//complex queries were getting me down:
	$bwlKeepQuerySimple=true;
	...
	if($_REQUEST[qpre . "parenttable"]!="")
	{
		$arrFK=RelationLookup($strDatabase, $_REQUEST[qpre . "parenttable"],  $_REQUEST[qpre . "launcherfield"],0);
	}
	$modeaddition="";
	...
	$strNameColumn=gracefuldecay($strDescrFields, firstnonidcolumname($strDatabase,  $strTable));
	$strAdminIDRangeAddendum=UserIDToSelectAddendum($strDatabase, $strTable, $strIDFieldName, $UserID);
	//echo $strAdminIDRangeAddendum;
 	$out="";
	...
	$strLimitingConditions=MergeLimitingConditionsWithRSData($strLimitingConditions, $recordforusewithwhereclause);
	$whereadditional =MergeSQLConditionals($strLimitingConditions, $whereadditional, false);
	...
	//echo $strSQL;
	$strRSSQL="SELECT " . $strCountSearch . "  FROM " .   $strDatabase. "." . $strTable . $strSQL;
	//echo $strRSSQL;
	$records = $sql->query($strRSSQL);
	$record=$records[0];
	...
	if (sql_error()!="" ||$bwlKeepQuerySimple)
	{
		...
		foreach ( $value as $key1 => $value1 )
		{
			...
			if (GreaterFieldDisplayLogic($strTable, $strFieldConfig, $key1, $intFieldCount, $intFieldLimitLo, $intFieldLimitHi, $display_field_list) && !$bwlForceHide || ($strSearchField==$key1))
			{
				...
				if ($bwlBrowseMode && $strNameColumn== $key1)  //we're in browser mode and this is a name column, so time for a hyperlink
				{
					...
					$strDataToDisplay="<a href=\"" . qbuild($strPHP, $strDatabase, $strOriginalTable , "view" . $modeaddition, $strIDFieldName, $value[$strIDFieldName]) . "&" . qpre . "displaytype=" . $displaytype . "&" . qpre . "filterid=" . $value[$strIDFieldName] .  $keyserialized . "\">" . strip_tags(trunchandler( $value1, $intTruncSize)) . "</a>";
				...
				{
					$strDataToDisplay=strip_tags(trunchandler($value1, $intTruncSize));
				}
				...
				//LabelForID($strDatabase, $strTable, $strIDField, $intID, $strKnownFields="")
				...
				//$strPickerLabel=LabelForID($strDatabase, $arrFKKeyToColumns[$columnname][0], $arrFKKeyToColumns[$columnname][1], $strDefault,$arrFKKeyToColumns[$columnname][4]);
			 	//LabelThis($strComputedPrettyFieldName, $strSetPrettyFieldNames, &$connector, $record="", $strLabelOverride="", $intTruncSize=40)
				$strPrettyName=LabelThis($strPickerLabel, gracefuldecay($arrFK[4], $strNameColumn), $connectorreturn, $value, "", $intTruncSize);
				//$strPrettyName=  $arrFKKeyToColumns[$key1][4] ;
				$intFieldCount++;
			}
		}
		if($bwlShowCountRelated)
...
//breaking this out into a separate function probably wasn't the wisest idea, but i had to get stuff out of DisplayDataTable
{
	...
	$out2.= "</td>\n";
	$strSearch.= HiddenInputs(array("parenttable"=>$_REQUEST[qpre . "parenttable"],"launcherfield"=>$_REQUEST[qpre . "launcherfield"], "column"=>$strSortColumn, "table"=>$strTable,"table"=>$strTable,"db"=>$strDatabase,"filterid"=>$intThisFilterID,"direction"=>$strDirection,"displaytype"=>$displaytype,"mode"=>"view","launcherfield"=>$launcherfield));
...
//this could use some trimming down
	...
	foreach ($descr as $nom=>$info)
	{
			...
			{
				$mrecord=$mrecords[0];
				$extra_functionality= $mrecord["extra_functionality"];
				...
				if ($strType=="int")
				{
					...
					if (count($arrFK)>0)
					{
						...
						else
						{
							...
							$strPickerLink.=" [<a href=\"javascript:pickerwindow('". $strDatabase . "','" . $arrFK[0] . "','".  $strFieldNamePrefix . $name ."';return false)\">browse</a>]\n";
	...
	if(!contains($strConfigBehave, "notopnav") && !contains($strConfigBehave, "nobottomnav") )
	{
...
//shows all the tables in $strDatabase
{
	...
	$out.=htmlrow("bgclassline", "<a href=\"javascript: SortTable('idsorttable', 0)\">table</a>", "<a href=\"javascript: SortTable('idsorttable', 1)\">records</a>",  
                "<a href=\"javascript: SortTable('idsorttable', 2)\">columns</a>", 
                 "<a href=\"javascript: SortTable('idsorttable', 3)\">engine</a>", 
                "other views", 
		...
		if (($bwlBeginsWithTF &&  $bwlSuperAdmin) || !$bwlBeginsWithTF)
		{
			...
			else
			{
				$tablemakerlink= "&nbsp;";
                        ...
                        $engine=getEngine($strDatabase, $tablename);
...
//a generic form generator that looks at the table's description in the db and then dynamically builds an editor form
	...
	$arrUniversalPK=$arrPK;
	if($arrPK=="")
	{
		$arrUniversalPK=Array($strIDField=>$strValDefault);
	}
	$out="";
	$preout="";
	$strValidationJS="";
	...
	$arrForceeditable[$nameforhelp]=Array();
	$arrForceuneditable[$nameforhelp]=Array();
	$arrIsDateCreated[$nameforhelp]=Array();
	$arrIsDateModified[$nameforhelp]=Array();
	...
	if ($strValDefault!=""  || $arrPK!="")
	{
		...
		$strSQL="SELECT * FROM " . $strDatabase . "." . $strTable . " WHERE " . ArrayToWhereClause($arrUniversalPK);
	...
	foreach ($descr as $nom=>$info)
	{
		...
		if(!inList($noshowfields, $name)  && $strKosherFields==""   || inList($strKosherFields, $name))
		{
			...
			{
				$mrecord=$mrecords[0];
				//echo "$" . $nameforhelp  . " " .  $mrecord["height"];
				$fieldlabel=gracefuldecay($mrecord["label"], $fieldlabel);
				...
				$arrForceeditable[$nameforhelp]=$mrecord["forceeditable"];
				$arrForceuneditable[$nameforhelp]=$mrecord["forceuneditable"];
			...
			else
			{
				...
				//where i ultimately set the default value for the form
				$strDefault=gracefuldecay($_GET[$name], $strDefault);
				if($arrIsDateModified[$nameforhelp])
				{
					$strOldModifiedDate=$strDefault  ;
					$strDefault=date("Y-m-d H:i:s"); //in cases where this is editable, i want to set it for now, 2010-09-23
					//$arrWidth[$nameforhelp]=gracefuldecay($arrWidth[$nameforhelp], 12);
				...
				if (count($arrFK)>0)
				{
					...
					//i capture this info in case i need it later for a MultiTable lookup
			 		...
			 		$arrRelationTemp[$arrFK[0]]=$strDefault;
					$skip=true; //here we skip because we have a dropdown
					$count = countrecords($strDatabase,$arrFK[0], $arrFK[6],$record);
					//if $count <101 do a dropdown for a foreign key.  otherwise slap down a link to a searchable picker
					//because i'm all cool like that
					...
					$max_dopdown=$arrFK[8];
					//echo "-" . $max_dopdown . "-";
					if($max_dopdown==""  || $max_dopdown==0)
					{
						$max_dopdown=300;
					}
					if ($count<$max_dopdown)
					{
						$fieldlabel=ReturnNonIDPartOfName($fieldlabel);
						...
						// foreigntablepulldown($strDatabase, $strTable, $strIDField, $intDefault, $strLabelField="", &$namereturn, $bwlHiddenReturn=false, $strPreferredNameField="", $addedselectpairs="", $strWhereClause="", $recordforusewithwhereclause="", $truncsize=35)
						$fieldform=foreigntablepulldown($strDatabase,$arrFK[0], $arrFK[1], $strDefault, $strFieldNamePrefix .$name, $bleh, false, $arrFK[4], "", $arrFK[6],$record, 35, $arrFK[9], $arrFK[10]);
						//idl-
					...
					else
					{
						...
						$strPickerLabel=LabelForID($strDatabase, $arrFK[0], $arrFK[1], $strDefault,$arrFK[4]);
						$strPickerLink= "<input style=\"font-size:9px; border:0px;\" class=\"" . $strThisBgClass . "\" type=\"text\" name=\"" . $strFieldNamePrefix .  qpre . "a|" .  $name . "\" size=\"20\" value=\"" . deescape($strPickerLabel)  . "\">\n";
						$strPickerLink.=" [<a href=\"javascript:pickerwindow('". $strDatabase . "','" . $arrFK[0] . "','".  $strFieldNamePrefix . $name ."','','','" . $strTable . "','" . urlencode(serialize($arrUniversalPK)) . "')\">browse</a>]\n";
				...
				if(probableserialized($strDefault))
				{
					...
					$height=gracefuldecay( $arrHeight[$nameforhelp], $height);
					...
					$skip=$fieldform!=""; //true; //here we skip because we use SerializeForm to make this part of the form
				}
				else if (inlist("bigint mediumint int smallint", $strType) || $strType=="tinyint" && endswith($name, "id"))
				{
				...
				elseif  (contains($strType,"text") || $intLen>100  && !(NeedsUpload($name)  || $arrIsFileUpload[$nameforhelp]==1))
				{
					...
					foreach($languagerecords as $languagerecord)
					{
						...
						else
						{
							...
							if($height<160/abs(count($descr)-2))
							{
								$height=abs(160/abs(count($descr)-2));
							}
							//echo $width;
				...
				if ($info["Extra"]!="auto_increment"  && !$skip)
				{
					...
					if($strTableLookingOutWith !="" && (!contains($name, "disp_column")  && contains($name, "column") || contains($name, "field")))
					{
					...
					elseif (contains($name, "table_name")) //special case for references to tables
					{
						...
						foreach($descr as $fnom=>$finfo)
						{
							...
							if ($bwlNextColumn  && (!contains($fname, "disp_column")  && contains($fname, "column") || contains($fname, "field")))
							{
					...
					else
					{
						...
						$fieldform=$strAboveForm . GenericInput($strFieldNamePrefix . $name ,  forinputform($strDefault) , false, "",   "", "", $strFormType, gracefuldecay($arrWidth[$nameforhelp],$width), "", $height, false). $fieldform . $strPickerLink;
						if($arrIsDateModified[$nameforhelp] && $arrForceeditable[$nameforhelp] )
						{
							$fieldform.=" (was " . $strOldModifiedDate . ")" ;
						}
					...
					if($arrForceuneditable[$nameforhelp] || ($arrIsDateCreated[$nameforhelp] || $arrIsDateModified[$nameforhelp]) && !$arrForceeditable[$nameforhelp] )
					{
				...
				elseif ($info["Extra"]=="auto_increment" && $strValDefault!="")
				{
					...
					if(!$arrForceeditable[$nameforhelp])
					{
				...
				$out.=$fieldlabel . "\n";
				if(contains($extra_functionality, "<value"))
				{
					$extra_functionality=str_replace("<value/>", $strValDefault, $extra_functionality);
					$extra_functionality=str_replace("<value>", $strValDefault, $extra_functionality);
				}
				$out.=$extra_functionality. "\n";
	...
	if(file_exists($strTable . ".edit"))//total groovy way to have your additional editors stuck at the bottom of the form;
	{
		//echo "DD";
		include $strTable . ".edit";
	}
   ...
   onmouseover=\"this.className='btn btnhov'\" onmouseout=\"this.className='btn'\" name=\"" . qpre . "submit\" type=\"submit\" value=\"" . $strButtonText ."\">\n";
	...
	$out.="<input type=\"hidden\" name=\"" . $strFieldNamePrefix . qpre . "dm\"   value=\"".  $strFieldNamePrefix .  $_REQUEST[qpre . "dm"] . "\">\n";                                                                   
	...
	if (!contains($strConfigBehave, "noextra")  && !contains($strConfigBehave, "notopnav") && !contains($strConfigBehave, "nobottomnav"))
	{
...
{
 	$value=trim($value);
	if((beginswith($value, "a:") || beginswith($value, "O:"))  && contains($value, ":{"))
	{
		//echo $value . "<BR>";
		return true;
...
function TableSearcherForm($strDatabase, $strTable,$strSearchString="", $strSearchField="", $intSearchType="", $strAdditionalSearchFields="" )
{
	$strPHP=$_SERVER['PHP_SELF'];
	$arrFilledOutAdditional=array();
	$sql=conDB();
 	$strSearch.="<form action='" .$strPHP . "' method='get'>";
	//if there is no data in the table then we have to look at the table's description
	$arrDesc=$samplerecord;
	if(count($arrDesc)<1)
	{
	...
	if( $strAdditionalSearchFields!="")
	{
		$arrAdditional=explode(" ", $strAdditionalSearchFields);
	}
	foreach($arrAdditional as $name)
	{
		$arrFilledOutAdditional[$name]="";
	}
	$strDirection=ReverseDirectionSQL($strDirection);
	...
	$strSearch.=   " the ";
	$strSearch.="<select name=\"" . qpre . "searchfield\" class=\"searchselect\">\n";
	$arrDesc =array_merge($arrDesc, $arrFilledOutAdditional);
	ksort($arrDesc );
	foreach ($arrDesc as $name=>$info)
	{
		$strSel="";	
	...
	$strSearch.="</select>\n field\n: <input class=\"btn\"
   onmouseover=\"this.className='btn btnhov'\" onmouseout=\"this.className='btn'\" name=\"" . qpre . "searchbutton\" value=\"search " .  PluralizeIfNecessary($strTable) . "\" type=\"submit\"></span></nobr>\n";
 	$strSearch.="</form>";
	return $strSearch;
}
?>