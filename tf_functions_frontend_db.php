	...
	if ($strSQL!="")
	{
		...
		if( count($rows)>0)
		{
			if ($strErrors=="" )
			{
				...
				foreach ($rows as $record )
				{
							...
							foreach($record as $k=>$v)
							{
								if($fieldcount<$intFieldLimit)
								{
									if(!inList($strSuppressFields, $k) && !$bwlSuppressHeader)
									{
										...
										{
											$out.="<a class=\"" .$strOtherLineClass . "\" href=\"javascript: SortTable('" . $tableid . "', " . $fielddisplaycount . ")\">" . $kdisplay . "</a>";
										}
										$out.="</td>";
										$fielddisplaycount++;
									}
						...
						if($fieldcount<$intFieldLimit)
						{
							if(!inList($strSuppressFields, $k))
							{
								...
								if (($fieldcount==0  && $strLinkFieldName==""  || $strLinkFieldName==$arrFN[$fieldcount])  && $strPHP!="")
								{
								  	...
								  	if($bwlNOQSPHP)
									{
										$body="<a ".$strTargetPair ."href=\"" .str_replace("<replace/>", $idtouse, $strPHP) .   "\">" . $body . "</a>";
									}
									else
									{
					...
					if ($strAdditionalLink!="")
					{
						...
						$strThisAdditionalLink= $strAdditionalLink;
				...
				{
					$out=$preout . TableEncapsulate($out, true, $intWidth, $tableid);
				}
			...
			else
			{
				...
				if(!$bwlNoTableEncapsulate)
				{
					$out.= "<p>" . TableEncapsulate($errorout, false,630, "iderrortable") . "</p>";
				}
			}
		}
	}
	...
	if (is_array($rows))
	{
		...
		if( count($rows)>0)
		{
							...
							foreach($record as $k=>$v)
							{
								if($fieldcount<$intFieldLimit)
								{
									if(!inList($strSuppressFields, $k) && !$bwlSuppressHeader)
									{
										...
										$out.="<a href=\"javascript: SortTable('" . $tableid . "', " . $fielddisplaycount . ")\">" . $kdisplay . "</a>";
										$out.="</td>";
										$fielddisplaycount++;
									}
								}
			...
			else
			{
				...
				if(!$bwlNoTableEncapsulate)
				{
					$out.= "<p>" . TableEncapsulate($errorout, false, $intWidth, "iderrortable") . "</p>";
				}
			}
		}
	}
...
function FrontEndTableForm($strDatabase, $strSQL, $strTable, $strIDField, $strPKValue, $strPHP,  $strHiddenList, $strDisplayOnlyList, $arrPassedInDefaults, $strConfigBehave, $strUser="", $bwlSuppressNewButton=false, $breadcrumb="", $bwlPlaintextPWD=false, $bwlSuppressSubmitButtons=false, $intWidth=630, $bwlSuppressTableNameOnButton=true, $bwlFriendlyLabels=false, $arrLabels="", $arrDisplayGuidance="", $arrExtraFields="" )
{
	//echo count($arrExtraFields);
// echo $strPKValue . "=<br>";
//a generic form generator that looks at the table's description in the db and then dynamically builds an editor form
	...
	{
		$descr=$descr[0];
		if(is_array($arrExtraFields)) //pass in form-processable fields beyond the table's definition!
		{
			//echo count($descr) . "<BR>";
			$descr=array_merge($descr, $arrExtraFields);
			//echo count($descr) . "<BR>";
		}
		...
		$record="";
		//total wrong-ass hacktron here, forgive me lord!
		foreach($_REQUEST as $k1=>$v1)
		{
			if(!beginswith($k1, qpre))
			{
				$record[$k1]=$v1;
			}
		}
		...
		foreach ($descr as $fieldlabel=>$info)
			{
				...
				if(!inList($noshowfields, $name))
				{
					...
					if (count($mrecords)>0)
					{
						...
						$arrExtra_functionality[$nameforhelp]=$mrecord["extra_functionality"];
						...
						$bwlHypLabel=false;
							...
							if (count($arrFK)>0)
							{
								...
								$bwlHypLabel=true;
									...
									$strNone="";
					 				$strGuidance="";
									//echo $arrDisplayGuidance[$nameforhelp] . "<BR>";
									$strGuidance=parseBetween($arrDisplayGuidance[$nameforhelp], "narrowing:'", "'", 0, true); 
									if ($count<101)
									{
										...
										if($arrDisplayGuidance[$nameforhelp]!="")
										{
											
											
											$strNone=parseBetween($arrDisplayGuidance[$nameforhelp], "none:'", "'", 0, true); 
											//echo $nameforhelp . ":" . $arrDisplayGuidance[$nameforhelp] . ":" . $strNone . "<BR>";
											if(contains($arrDisplayGuidance[$nameforhelp], "none:")  && $strNone=="")
										...
										//echo $strGuidance;
										//$strGuidance allows me to pass in where clauses from the front end to narrow dropdowns to some condition
								
								
	//foreigntablepulldown($strDatabase, $strTable, $strIDField, $intDefault, $strLabelField="", &$namereturn, $bwlHiddenReturn=false, $strPreferredNameField="", $addedselectpairs="", $strWhereClause="", $recordforusewithwhereclause="", $truncsize=35, $strSortColumn="", $bwlSortDescending=false, $strNone="none")
										$fieldform=foreigntablepulldown($strDatabase,$arrFK[0], $arrFK[1], $strDefault, $name, $bleh, false,$arrFK[4], "",gracefuldecay($strGuidance,  $arrFK[6]),$record, 35, $arrFK[9],$arrFK[10], $strNone);
					...
					if  ($strType=="text" || $intLen>100)
					{
						...
						if($arrDisplayGuidance[$nameforhelp]!="")
						{
							$width=gracefuldecay(parseBetween($arrDisplayGuidance[$nameforhelp], "width:", " ", 0, true), $width);
							$height=gracefuldecay(parseBetween($arrDisplayGuidance[$nameforhelp], "height:", " ", 0, true), $height);
						}
					...
					elseif  ($strType=="date"  || (contains($name, "datecode")  && function_exists("makedate")))//datecodes are ints of the form YYMMDD
					{
						...
						$skip=true;
					 	//datepulldowns($strNamePre,$dtmDefault, $bwlDontShowNone=false, $strFormatString="", $yearlow="", $yearhi="", $bwlCCDate=false, $bwlYearOnly=false)
						$bwlYearOnly=false;
						if($arrExtra_functionality[$nameforhelp]=="YearOnly")
						{
							$bwlYearOnly=true;
						}
						//echo "-" .   $bwlYearOnly . "-";
						$fieldform=datepulldowns( $name, $strDefault, false, "", "", "", false, $bwlYearOnly);
					}
	 ...
	 name=\"" . qpre . "submit_clearid\" type=\"submit\" value=\"" . $strOtherButtonText ."\">\n";
		...
		$out= TableEncapsulate($out, false, $intWidth, "idtablefive");
...
function GenericFormSaver($strDatabase, $strTable, $strPrimaryKey, $strPKVal, $strFieldIgnoreList, $bwlDebug=false, $bwlWorkFromSchema=false, $bwlDontOverwriteWithNulls=true, $bwlCreateSchema=false, $strImageUploadKeyFieldName="", $strFieldForceRead="", $strUploadImageFolderName="", $intMaxWidth=0, $intMaxSize=0)
...
//judas gutenberg
{
	...
	{
		if( 1==1 || $placetogetdata==$_REQUEST  || $placetogetdata==$_POST  || $placetogetdata==$_GET || $placetogetdata==$_COOKIE)//actually always do this
		{
			$v=removeslashesifnecessary($v);//deal with fucking get_magic_quotes_gpc nightmare!!
		}
		//echo $k . " " . $v . "<br>";
		if($k!="MAX_FILE_SIZE")
		{
			...
			else if ($_FILES[qpre . "u|" . $k]["name"]!="")
			{
				...
				$filename=str_replace(" ", "_", $_FILES[qpre . "u|" . $k]["name"]);
				//intMaxSize
				//intMaxWidth
				$imageinfo =getimagesize($_FILES[ qpre . "u|" . $k]['tmp_name'] );
				$intwidth =$imageinfo[0];
				$intfilesize=$_FILES[qpre . "u|" . $k]['size'];
				if($intfilesize> $intMaxSize  && $intMaxSize>0)
				{
					$out.="The uploaded file was  " . intval(($intfilesize * 100) / $intMaxSize-100) .  "% too big.<br>";
				}
				
				else if($intwidth> $intMaxWidth  && $intMaxWidth>0)
				{
					$out.="The uploaded file was  " . intval(($intwidth * 100) / $intMaxWidth-100) .  "% too wide.<br>";
				}
				else
				{
					$destpath=fieldNameToFolderPath( $k, $strUploadImageFolderName)  . $filename;
					//echo "------" . $destpath;
					if (!BuildPathPiecesAsNecessary($destpath))
					{
						$out.="The path " .$destpath . " could not be built.<br>";
					...
					foreach($arrTables as $strThisTable)
					{
						...
						if($bwlWorkFromSchema && GetFieldType($strDatabase, $strThisTable, $k)== "date")
						{
							...
							if (GetFieldType($strDatabase, $strThisTable, $k)== "date")
							{
								...
								if(!inList($fieldsadded[$strThisTable], $k))
								{
									$arrIncidental[$strThisTable][$k]=singlequoteescape($v);
									$fieldsadded[$strThisTable] .=" " . $k;
								}
							}
						...
						else
						{
							...
							else if($v!=""  && $strPKVal=="")
							{
								...
								if(!inList($fieldsadded[$strThisTable], $strPrimaryKey))
								{
										$arrID[ $strPrimaryKey]=($v);
										$fieldsadded[$strThisTable].=" " . $strPrimaryKey;
								}
								//echo $k . " " . $v . "-id<br>";
							}
						}
