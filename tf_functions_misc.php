...
function prevnextlinks($strDB, $strThisTable, $sortcolumn, $id_column, $strLabel,   $type, $strAdditionalField="", $strAdditionalValue="", $thiscolumn, $bwlJustQS=false, $strIncludeFields="")
{
	...
	$strBaseSQL="select  * from " . addDatabaseIfNecessary($strDB, $strThisTable) .  " where " . $sortcolumn;
	if ($type=="newer")
	{
...
function tfJSONDump($strDatabase, $strTable, $strFields="", $strFieldDelimiter=",", $strRowDelimiter="\n", $quotecontent=true, $UserID="", $strSQL="")
...
//If $strFields is not empty, then the fields are filtered based on their mention in $strFields
{
	...
	if ($strSQL=="")
	{
		$strSQL="SELECT * FROM  " . addDatabaseIfNecessary($strDatabase, $strTable);
	
		$strSQL=ProperlyAppendSQLWherePhrase($strSQL, $strAdminIDRangeAddendum);
	}
	$records = $sql->query($strSQL);
	 
	return  json_encode(Array("items"=>$records));
}
...
//If $strFields is not empty, then the fields are filtered based on their mention in $strFields
{
	...
	foreach ($records as $key=>$value )
	{
		...
		foreach ($value as $key1 => $value1 )
		{
			...
			$strpreparedcontent=str_replace($strRowDelimiter, " ", doublequoteescape($value1));
			if ($quotecontent)
			{
...
//although this thing is pretty complicated now!
{
	...
	$out.= "<form enctype=\"multipart/form-data\" method=\"post\" name=\"" . $formname . "\" action=\"" .  $strPHP . "\" " . $formextratags . ">\n";
	...
	for($i=0; $i<count($arrFieldNames); $i++)
	{
		...
		if(contains($thisconfig, "styleinput:"))
		{
			$inputstyle=parseBetween($thisconfig, "styleinput:", " ", 0, true);
		}
		if(contains($thisconfig, "classinput:"))
		{
			$inputclass=parseBetween($thisconfig, "classinput:", " ", 0, true);
		...
		if (contains($thisconfig,"label:"))//you can pass in a label this way, but you have to use underscores instead of spaces these get converted to spaces
		{
 		...
 		if (contains($thisconfig,"noformitem"))
		{
			$noformitem=true;
		...
		if (contains($thisconfig,"style:"))
		{	
	
			$class=parseBetween($thisconfig, "style:", " ", 0, true);
			//echo $thisconfig . "*" . $class . "<BR>";
		}
		...
		else
		{
			...
			else
			{
				...
				{	
					...
					if($bwlNumberDropdown)
					{
						...
						{
							if(genericdata($thisfieldname,0, 0, $strRequiredConfig, "~", "|")==$strName)
							{
					...
					else
					{
						...
						$strNoneEquiv=gracefuldecay($arrThisFieldName[5], "none");
						if($strNoneEquiv==" ")
						{
							$strNoneEquiv="";
						}
						//echo "+" . $strThisDefault . "+<br>";
						//echo $thistablename;
						//foreigntablepulldown($strDatabase, $strTable, $strIDField, $intDefault, $strLabelField="", &$namereturn, $bwlHiddenReturn=false, $strPreferredNameField="", $addedselectpairs="", $strWhereClause="", $recordforusewithwhereclause="", $truncsize=35, $strSortColumn="", $bwlSortDescending=false, $strNone="none")
						
						//echo $strThisDefault . "=" . $thistablepk . "<BR>";
						$inputtag=foreigntablepulldown(our_db,  $thistablename, $thistablepk, $strThisDefault, $thislocalfieldname, $namereturn,  false, $thistablelabelfield, $strNoneEquiv, $whereclause, "", 35, "", false, $strNoneEquiv);


						$strRequiredIndication="";
				 		if($strRequiredConfig!="")
						{
							if(genericdata($thisfieldname,0, 0, $strRequiredConfig, "~", "|")==$thislocalfieldname)
							{
								$strRequiredIndication="<span class='requiredindication'>*</span>";
								$intRequired++;
							}
				...
				{	
					...
					else if($type=="file_keytorecord")
					{
						$inputtag= "<input type=\"hidden\" name=\"" . qpre . "upload_keytorecord_" . $thisfieldname . "\" value=\"".  $strThisDefault . "\">\n";
						$inputtag.= GenericInput($thisfieldname , $strThisDefault,  false,  "",   $inputstyle,  $inputclass, "file",$size, "", $height) ;
					}
					else if($type=="date")
					{
					...
					{
						$inputtag=datepulldowns($thisfieldname,$strThisDefault,false, "", date("Y"),date("Y")+20, true);
					}
		 			else if($type=="yearonlypast")
					{
				 
						$inputtag=datepulldowns($thisfieldname,$strThisDefault,false, "", date("Y")-100,date("Y")+1, false, true);
					}
					else if($type=="yearonlyfuture")
					{
				 
						$inputtag=datepulldowns($thisfieldname,$strThisDefault,false, "", date("Y")-1,date("Y")+100, false, true);
					}
					else
					{
						$inputtag= GenericInput($thisfieldname, $strThisDefault,  false,  "",   $inputstyle,  $inputclass, $type,$size, "", $height) ;
					}
				}
			}
			if($noformitem)
			...
			if($action!="")
			{
			//echo  "<alert>script('" . $action . ")</script>";
			}
			//echo  $infostring . "<BR>";
			if($bwlAllOneRow  )
			
			{
				$rowcontent.=  gracefuldecay($foundlabel,$arrLabels[$i], $thisfieldname) .$strRequiredIndication . "&nbsp;" .  $inputtag . $infostring;
				if($bwlAllOneRow )
				{
					$out.=$rowcontent;
					$rowcontent="";
			...
			else
			{
				...
				$rowcontent.=gracefuldecay($foundlabel, $arrLabels[$i], $thisfieldname) . $strRequiredIndication ;
				$out.=htmlrow($classToUse, $rowcontent , $inputtag . $infostring);
				
				$rowcontent="";
				//echo $classToUse . " " . $thisfieldname . "<BR>";
...
//returns a table's checksum from mysql
{
	...
	$strSQL="ALTER TABLE " .  addDatabaseIfNecessary($strDatabase, $tablename) . " CHECKSUM=1";
	$rs = $sql->query($strSQL);
	//echo sql_error(). "<br>";
	$strSQL="CHECKSUM TABLE " . addDatabaseIfNecessary($strDatabase, $tablename);
	$rs = $sql->query($strSQL);
	$r=$rs[0];
	$out=$r["Checksum"];
...
//i pass back tfrecords to make it usable to subsequent calls
{
	...
	foreach($tfrecords as $record)
	{
		...
		if($strPK!="")
		{
			$cstrSQL="SELECT count(" . $thiscolumn  . ") AS thiscount FROM " . addDatabaseIfNecessary($strDatabase, $thistable) . " WHERE " . $thiscolumn . "='" . $strPK . "'";
			//echo $cstrSQL  ."<br>";
	 		//echo mysql_error()  ."<br>";
...
function LabelForID($strDatabase, $strTable, $strIDField, $intID, $strKnownFields="")
{
	...
	{
		$sql=conDB();
		$strSQL="SELECT " . join(",", $arrNameFields) . " FROM " . addDatabaseIfNecessary($strDatabase, $strTable)  . " WHERE " .  $strIDField . " = " . $intID;
		$records = $sql->query($strSQL);
		if ($records)
		{
			//echo"@";
...
function foreigntablepulldown($strDatabase, $strTable, $strIDField, $intDefault, $strLabelField="", &$namereturn, $bwlHiddenReturn=false, $strPreferredNameField="", $addedselectpairs="", $strWhereClause="", $recordforusewithwhereclause="", $truncsize=35, $strSortColumn="", $bwlSortDescending=false, $strNone="none")
...
{
	//echo "<BR> WHERE: " . $strWhereClause;
	//echo "<p><br><br>";
	//var_dump( $recordforusewithwhereclause);
	//echo "<p>";
 	//echo $strWhereClause;
	//echo  $strTable  . " " . $strIDField ." " . $intDefault .  "<BR>";
	//echo   $strIDField . " " . $intDefault.  "<BR>";
	//var_dump($recordforusewithwhereclause);
	$sql=conDB();
	$strHiddenExtra="";
	$strNameField2="";
	...
	if($strNone!="")
	{
		$strOut.="<option value=".chr(34).chr(34).">" . $strNone .  "\n";
	}
	...
	if( $strIDField!="" && is_array($arrLabelFields))
	{
		$fieldlist= "a." . $strIDField  ;
		//echo count( $arrLabelFields);
		if(count( $arrLabelFields)>0)
		{
			$fieldlist.=",a.". join(",a.", $arrLabelFields);
		}
	}
	else
	{
	...
	$strSQL="SELECT " . $fieldlist . " FROM " . addDatabaseIfNecessary($strDatabase, $strTable). " a ";
	...
	//echo  "<BR>=" . $strSQL;
	$strSortColumn=gracefuldecay($strSortColumn, $orderby, $strNameField);
	if ($strNameField!="")
	{
		$strSQL.=" ORDER BY " . str_replace(" ", ",", $strSortColumn);
		if($bwlSortDescending)
		{
			$strSQL.=" DESC";
		}
	}
	...
	//}
	//echo our_db . " " . $strSQL . "<BR>";
	$records = $sql->query($strSQL);
	if(sql_error() !="")
	{
		return "An error was thrown by the dropdown generator. Perhaps narrowing conditions need to be changed.";
	}
	if ($records)
	{
		$strOldLabel="";
...
function LookupAPotentialLimitingJoin($strDatabase, $strOurTable, $strMysteryColumn, $strTableToAvoid)
{
	...
	$strSQLOUT="SELECT * FROM " .  addDatabaseIfNecessary($strDatabase, $strOurTable)  . " INNER JOIN   " .$strDatabase . "." .$maptablename . "  ON   " . addDatabaseIfNecessary($strDatabase, $strOurTable) . "." . $maptablePK  . "=" .   $strDatabase . "." . $maptablename  . "." .  $columnonothersideofmap;
	return $strSQLOUT;
}
...
//cross-relation selects
{
	...
	foreach($records as $record)
	{
		...
		{
			$strJoinSQL.=" LEFT JOIN " . addDatabaseIfNecessary($strDatabase, $record["f_table_name"])  . " x" . $count. " ON t." . $record["column_name"] . "=x" . $count. "." . $record["f_column_name"] . " " ;
			$suggestedcolumn="";
			$arrSuggestion="";
			if($arrFKNameMap=="")
			{
	...
	$out="SELECT " . $strDISTINCT .   $goodFieldList .  IFAthenB(($strPreJoinSQL  && $goodFieldList),", ") . $strPreJoinSQL  ." FROM " .   addDatabaseIfNecessary($strDatabase, $strTable)  . " t " . $strJoinSQL . " " .   " " . $additionalClauses;
	if($strPK!="")
	{
...
//$bwlAcceptWild allows it to add items at the bottom found in the wild but not in the config string
{
	...
	if($strSQL=="")
	{
		$strSQL="SELECT * FROM " . addDatabaseIfNecessary($strDatabase, $strTable) . " ORDER BY " . $strNameField . " ASC";
	}
	$records = $sql->query($strSQL);
...
function WhoIsLoggedIn()
{
	...
	else if(authtype!="xcart"  || !defined("authtype"))
	{
		if ($strCookieVal!="")
		{
			$strCookieVal =GenericDBLookup(our_db, $a["table"], $a["username"], $strCookieVal,$a["username"],  false,  true);
			return($strCookieVal);
		}
...
function IsLoginValid($strDatabase, $strUser, $strPassword)//can't use $strDatabase!!
{
	$a=GetLoginTableArray();
	$out=false;
	$sql=conDB();
	//use the function data to do a query 
 	$strSQL="SELECT " . $a["password"] . " from " . our_db . "." . $a["table"] . " WHERE " . $a["username"] . " = '" . $strUser . "'";
	//echo $strSQL;
	$records = $sql->query($strSQL);
	//echo mysql_error();
...
function TestAndSetLogin($strDatabase, $strUser, $Password)//can't use $strDatabase!!
//If login is valid, set the cookie and dude is logged in.
{                                                   
	if (IsLoginValid(our_db, $strUser, $Password))
	{
		setLoggedIn($strUser);
		$out=$strUser;
	}
...
/////more complex user functions:


function setLoggedIn($strUser, $years="")
...
{                
	...
	if(beginswith(authtype, "code:"))
	{
		...
		setcookie($a["cookie"], $cookievalue, time()+ $lifespan, "/" );
		//echo $cookievalue;
	}     
	else if(authtype!="xcart"  || !defined("authtype"))
	{
		//die($strUser);
		if ( setcookie($a["cookie"], addletters($strUser, encryptionkey),intval( time()+ $lifespan), "/" ))
		{
			//echo "cookie set!";
		}
	}
	...
	else
	{
		...
		else
		{
			...
			if(!setcookie($a["cookie"], $sessid, intval( time()+ $lifespan)))
			{
...
function loginarrest($strPHP, $mode="")
//Throws up a login form if the user is not logged in.
{
	...
	if($mode=="minimal")
	{
		$out.="[<a href=# onclick='var x=document.getElementById(\"idloginswitcher\");x.style.display=\"\";var x=document.getElementById(\"idloginpiece\");x.style.display=\"none\"'>hide</a>]\n";
	}
	$out.="<input type=\"Submit\" value=\"Login\"></td></tr>\n";
	...
	if($mode=="minimal")
	{
		$out="<div id='idloginpiece' style='display:none'>\n" . $out;	
 
		$out.="</div>\n";	
		$out.="<div id='idloginswitcher' >\n";
		$out.="[<a href=# onclick='var x=document.getElementById(\"idloginpiece\");x.style.display=\"\";var x=document.getElementById(\"idloginswitcher\");x.style.display=\"none\"'>login</a>]\n";
		$out.="</div>\n";
	}
	return $out;
}

function LoginDecisions($strDatabase,  $strPHP,  &$strUser, $bwlSimpleOutput=false, $strConfigBehave="")//can't use strDatabase
...
//Returns $strUser as a "by reference" variable
{
	...
	if(contains($strConfigBehave,"minimal"))
	{
		$mode="minimal";
	}
	...
	//echo "-" . $strUser . "+" . $Password . "+";
	if (countrecords(our_db ,$a["table"])<1)
	{
		$strUser="-no security-";
		$skiplogintest=true;
	...
	elseif ($strUser!="" && $Password!=""   )
	{
		...
		$strUser=trim(TestAndSetLogin(our_db, $strUser, $Password));
		//echo "+" . $strUser . "+<BR>";
		if ($strUser==""  && $Password!="")
		{
	...
	//echo "-" . $strUser . "-" . $Password;
	$bwlSuperAdmin=IsSuperAdmin(our_db, $strUser);
	//echo "#" . $bwlSuperAdmin  . "&";
	if (!$bwlSimpleOutput )
	{
		if ($strUser!="")
...
function IsSuperuser($strDatabase, $strAdmin)//can't use strDatabase
//Is this TF user a super user?  This is powerful and allows all activities, but certain admin tasks are kept hidden.
{
	$a=GetLoginTableArray();
	//echo "-" . $a["table"];
	if (TableExists(our_db, tfpre . "permission") || TableExists(our_db, $a["table"])  || superuserexplicit) //if no admin tables, everyone is an admin!
	{
		

		$sql=conDB();
		
		$strSQL="SELECT  * from  " . our_db . "." . $a["table"] . " WHERE " . $a["username"] . " = '" .   singlequoteescape($strAdmin) . "'";
	 	//echo $strSQL;
		$records = $sql->query($strSQL);
		...
		if (count($records)>0)
		{
			...
			if(is_string($a["superuserindicationvalue"]))
			{
				if(contains($record[$a["superuserindicationfield"]], $a["superuserindicationvalue"]))
				{
					$out=true;
				}
			}
	...
	$out=$out || IsSuperAdmin(our_db, $strAdmin);
	//echo "-" . $out . "-";
	return $out;
}

function IsSuperAdmin($strDatabase, $strAdmin) //can't use strDatabase
//Is this TF user a super admin?  This is the highest level of power in the system.
{
	$a=GetLoginTableArray();
	$sql=conDB();
	if(TableExists(our_db, $a["table"]))
	{
		$strSQL="SELECT  * from  " . our_db. "." . $a["table"] . " WHERE " . $a["superadminindicationfield"] . " = '" . $a["superadminindicationvalue"] . "'";
		//echo $strSQL;
		$records = $sql->query($strSQL);
		//echo "count_adminpossible:" . count($records) . "-";
		if (count($records)>0  && (authtype!="xcart"  || !defined("authtype")))  //if there are no superadmins then everyone is a superadmin!
		{
			
			$strSQL="SELECT  " . $a["superadminindicationfield"] . " from  " . our_db . "." . $a["table"] . " WHERE " . $a["username"]. " = '" . singlequoteescape($strAdmin) . "'";
			//echo $strSQL;
			$records = $sql->query($strSQL);
			$record=$records[0];
			...
			if(is_string($a["superadminindicationvalue"]))
			{
				//if $a["superadminindicationvalue"] is CS and $record[$a["superadminindicationfield"]] is CSA then CS has been found
				if(contains($record[$a["superadminindicationfield"]],$a["superadminindicationvalue"]))
				{
					$out=true;
				}
			}
		...
		{
			$strSQL="SELECT   usertype from  " . our_db . "." . $a["table"] . " WHERE usertype='P' AND " . $a["username"]. " = '" . singlequoteescape($strAdmin) . "'";
			$records = $sql->query($strSQL);
			if(count($records)>0)
			{
				$out=true;
...
//Creates the SQL phrase added to the select in cases where 
{
	if ($UserID!="")
	{
		...
		$sql=conDB();
		$strSQL="SELECT id_range_lowend, id_range_highend FROM " . our_db . "." . tfpre . "permission WHERE admin_id = " . singlequoteescape($UserID) . " AND  table_name='" . $strTable . "'";
		//echo $strSQL;
		$rs = $sql->query($strSQL);
		$r=$rs[0];
		$lowpart="";
		$highpart="";
		if ($r["id_range_lowend"]!=""   &&   $r["id_range_lowend"]>0)
		{
			$lowpart=$pk . ">=" . $r["id_range_lowend"];
		}
		if ($r["id_range_highend"]!="" &&   $r["id_range_highend"]>0)
		{
...
function AdministerType($strDatabase, $strTable, $strAdmin, $id="", $user_id="") //can't use strDatabase
//Returns an integer determining what a TF user is allowed to do.
//2 is lots of power, 0 is none.  1 means read but no write.
{
	if( $user_id!="")
	{
		$strAdmin= GetAdminName(our_db, $user_id);
	}
	//die("-" . $strAdmin);
	$a=GetLoginTableArray();
	$sql=conDB();
	$strSQL="SELECT permission_type_id as 'p', id_range_lowend, id_range_highend FROM  " . our_db . "." . tfpre . "permission p LEFT JOIN " . $strDatabase . "." . tfpre . "admin a ON p.admin_id=a.admin_id WHERE a." .$a["username"] . " = '" . singlequoteescape($strAdmin) . "' AND p.table_name='" . $strTable . "'";
	$bwlSuperAdmin=IsSuperAdmin($strDatabase, $strAdmin);
	$bwlSuperUser=IsSuperuser($strDatabase, $strAdmin) || $bwlSuperAdmin;
	
	if (TableExists(our_db, tfpre . "permission") && TableExists(our_db, $a["table"]) )
	{
		$rs = $sql->query($strSQL);
		//echo count($rs);
		$r=$rs[0];
	...
	if(inList(superadmintables, $strTable)    || superusertables=="*" )
	{
		if($bwlSuperAdmin)
		{
			$out=2;
	...
	if(inList(superusertables, $strTable)   || superusertables=="*")
	{
	...
	if($strTable=="" && !$bwlSuperAdmin  && !$bwlSuperUser)
	{
		$out=0;
	}
	//echo $out . "=<BR>";
...
function GetAdminID($strDatabase, $strAdmin) //can't use strDatabase
...
//authentication is configured in constants.php.
{
	$a=GetLoginTableArray();
	$sql=conDB();
	$strSQL="SELECT " .$a["user_pk"] . " FROM  " . our_db . "." . $a["table"] . "  WHERE " . $a["username"]. " = '" . singlequoteescape($strAdmin) . "'";
	$records = $sql->query($strSQL);
	$record=$records[0];
...
function GetAdminName($strDatabase, $user_id) //can't use strDatabase
//Given the name of an TF user, return the user_id.  Could look in different tables depending on how
//authentication is configured in constants.php.
{
	$a=GetLoginTableArray();
	$sql=conDB();
	$strSQL="SELECT " .$a["username"] . " FROM  " . our_db . "." . $a["table"] . "  WHERE " . $a["user_pk"]. " = " .intval($user_id) ;
	//echo $strSQL;
	$records = $sql->query($strSQL);
	$record=$records[0];
	$out= $record[$a["username"]];
	//die($out);
	return $out;
}
...
function CreateTableFromSQLFile($strDatabase, $strTableName)//might have to look at this for addDatabaseIfNecessary($strDatabase, $strTable)
{
...
function PictureIfThere($strPath, $intWidth, $intHeight="", $border=" ", $url="", $alt="")
	...
	if (tf_file_exists($path))
	{
		if (isImageFileName($strPath))
		{
			...
			else
			{
				...
				if($alt!="")
				{
					$altpart="alt='" . addslashes($alt) . "' ";
				}
				$out.=  "<img ". $altpart.  $stupidiestylehack ." src=\"" . $strPath  ."\" " . IFAThenB($intWidth, "width=\"" . $intWidth . "\"") ." " .   $intHeight . " border=\"0\">";
			}
		}
	}
	//echo $out . "<P>";
...
//Display a Flash object embed for both IE and Mozilla
{
	...
	if ($canflash==true)
	{
		...
		if(1==2)
		{
			...
			$out.= " id=\"idswf". $swffile . "\"  width=\"".$width."\" height=\"".$height."\">\n";
			...
			//embed if for IE:
			$out.="<embed allowscriptaccess='always' base='.' menu='false' id=\"ideswf". $swffile . "\" wmode=\"transparent\" src=\"". $swffile . "\" quality=\"high\"\n";
		...
		{
			$rndnum=mt_rand(0,9000);
			$out.= "<div id=\"flashcontent" . $rndnum. "\" ></div>";
			$out.= "<script type=\"text/javascript\">\n";
			$out.= "// <![CDATA[\n";
			$out.= "var so = new SWFObject(\"" . $swffile. "\", \"main\", \"" . $width . "\", \"" . $height . "\", \"9\", \"#".$bgcolor."\");\n";
			$out.= "so.addParam(\"wmode\", \"transparent\")\n";
			$out.= "so.write(\"flashcontent" . $rndnum. "\")\n";
			$out.= "// ]]>\n";
			$out.= "</script>\n";
			...
			if ($strPathOut!="")
			{
				...
				//don't add a file or folder if this item contains a "."
					...
					if (!mkdir($strPathOut, 0777))
						{
							$out=false;
							echo "<div style='color:#990000'>" . $strPathOut . "</div>";
						}
				}
			}
		}
...
//clears all data from a table
	...
	if(CanChange())
	{
		if (contains($strTable, " "))//handle multiple empty - dangerous!
		{
			...
			{
				$strSQL= "TRUNCATE TABLE " . addDatabaseIfNecessary($strDatabase, $strTable);
...
//Removes a table completely from the db.
{
	...
	if(CanChange())
	{
		$strSQL= "DROP TABLE " . addDatabaseIfNecessary($strDatabase, $strTable);
		$tables = $sql->query($strSQL);
...
//Deletes a row of data from $strTable in $strDatabase matching $idfield=$id
{
	...
	if (($bwlBeginsWithTF &&  $bwlSuperUser) || !$bwlBeginsWithTF)
	{
		...
		if(is_array( $arrPK))
		{
			...
			if(CanChange())
			{
				DeleteBySpec($strDatabase, $strTable, $arrPK);
				//$records = $sql->query($strSQL);
		...
		else
		{
			...
			if(CanChange())
			{
				DeleteBySpec($strDatabase, $strTable, Array($idfield=>$id));
				//$records = $sql->query($strSQL);
				$out= "A row was deleted from the " . $strTable . " table in the " . $strDatabase . " database whose " .  $idfield . " was equal to " . $id;
			}
			else
...
function tf_mail($to, $subject, $message, $additional_headers, $additional_parameters)
//wrapper function for times when the site email needs to be fixed fast!
{
	$bwlRegularMail=false;
	...
	if($bwlRegularMail)
	{
		//echo "#" . $additional_parameters . $additional_headers;
		//echo "-" . mail($to, $subject, $message, $additional_headers, $additional_parameters) . "-";
		$bwlTest=false;
		if($bwlTest)
		{	
			...
			echo "additional_parameters:" . $additional_parameters . "<BR>";
		}
		$olderror=error_reporting(0);
	...
	{
		if(contains(mailer_url, "/"))
		{
			$urltouse=mailer_url;
			if(contains(strtolower(mailer_url), "http://"))
			{
				$urltouse=substr(mailer_url, 7);
				
			}
			$firstslashpos=strpos($urltouse, "/");
			$host=substr($urltouse, 0, $firstslashpos);
			$path=substr($urltouse,$firstslashpos);
		}
	 	$data="";
		$data.="to=" . urlencode($to);
		$data.="&subject=" . urlencode($subject);
		$data.="&body=" . urlencode($message);
		$data.="&headers=" . urlencode($additional_headers);
		$data.="&more=" . urlencode($additional_parameters);
		$data.="&auth=" . mailer_auth;
		//die($host. "<BR>" .  $path . "<BR>" . $data);
		$out=httpSocketConnection($host, "POST", $path, $data);
		//echo "-" . $out . "-";
		return intval($out);
	}
