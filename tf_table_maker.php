...
function main()
	{
		...
		$strAfterMakeForkURL=$_REQUEST[qpre . "forkurl"];
		$strAfterMakeForkTarget=$_REQUEST[qpre . "forktarget"];
		$strAfterMakeForkTasks=$_REQUEST[qpre . "postforkjstasks"];
			...
			if ($intAdminType>1)
				{
				 	if ($mode=="save")
					{
						...
						//echo "!<BR>";
						if($strAfterMakeForkURL!="")
						{
							$strAfterMakeForkURL=str_replace("<table/>", $strTable, $strAfterMakeForkURL);
							//echo "<BR><B> "  . $strAfterMakeForkURL . "</B><BR>";
							$out.="\n<script>\n" .  $strAfterMakeForkTarget . "='" .  $strAfterMakeForkURL ."';\n";
							//$out.="if(opener){\n";
							//$out.="if(opener.BForm){\n";
							//$out.="if(opener.BForm['" . qpre . "newtablename']){\n";
							//$out.="opener.BForm['" . qpre . "newtablename'].value='" . $strTable . "';\n";
							//$out.="}\n";
							//$out.="}\n";
							//$out.="}\n";
							//echo $strAfterMakeForkTasks . "=<P><font color=red>";
							$strAfterMakeForkTasks=str_replace("<table/>", $strTable, $strAfterMakeForkTasks);
							if($strOldTableName!=$strTable)
							{
								$out.="opener.document.BForm['" . qpre . "taskstodo'].value='RemoveTable(\"" . $strOldTableName . "\")';\n";
							}
							else
							{
								$out.="opener.document.BForm['" . qpre . "taskstodo'].value='RemoveTable(\"" . $strTable . "\")';\n";
							}
					 
							//echo $strAfterMakeForkTasks . "</font><P>";
							$out.=$strAfterMakeForkTasks . ";\n";
					...
					$out.= TableMaker($strDatabase, $strTable, $strTypeConfig, $bwlAdvanced, $strAfterMakeForkURL, $strAfterMakeForkTarget, $strAfterMakeForkTasks);
...
function TableMaker($strDatabase, $strTable, $strTypeConfig, $bwlAdvanced=false, $strAfterMakeForkURL="", $strAfterMakeForkTarget="", $strAfterMakeForkTasks="")
{
	...
	if ($strTable!="")
	{
		...
		foreach ($descr as  $info)
		{
			...
			if(count($rs)>0)
			{
				...
				$extra_functionality=$r["extra_functionality"];
				...
				$forceeditable=$r["forceeditable"];
				$forceuneditable=$r["forceuneditable"];
				$password=$r["password"];
				$width=$r["width"];
				$height=$r["height"];
			}
			...
			$strUpDownArrows=UpDownArrows($outcount);
			$out.=FieldEditLine($strDatabase, $strTable,  $outcount,$name,$strType,$intLen, $default, $bwlNull,$bwlPK,  $bwlAI,  $friendly, $helptext,$extra_functionality, $validation_type_id,$validation_pattern_id, $invisible, $force_wysiwg, $force_no_wysiwg, $force_binary, $force_text_input, $fileupload, $datecreated, $datemodified, $forceeditable, $forceuneditable, $password, $width, $height, $strTypeConfig, $strUpDownArrows, $strThisBgClass, $strStyleAdvanced,   $autoinccheckboxstyle, $strStyleAdvanced, $rid, $bwlIsPK);
	...
	$out.=HiddenInputs(array("mode"=>"save",  "db" =>$strDatabase, "fieldcount"=>$outcount, "delete"=>"",  "repositions"=>"", "oldtablename"=>$strTable, "forkurl"=>$strAfterMakeForkURL, "forktarget"=> $strAfterMakeForkTarget, "postforkjstasks"=>$strAfterMakeForkTasks));
...
function FieldEditLine($strDatabase, $strTable, $outcount,$name,$strType,$intLen, $default, $bwlNull,$bwlPK,  $bwlAI, $friendly, $helptext, $extra_functionality, $validation_type_id,$validation_pattern_id, $invisible ,  $force_wysiwg , $force_no_wysiwg ,   $force_binary , $force_text_input, $fileupload ,  $datecreated , $datemodified, $forceeditable, $forceuneditable, $password , $width , $height, $strTypeConfig, $strUpDownArrows, $strThisBgClass, $strStyleAdvanced,   $autoinccheckboxstyle, $strStyleAdvanced, $intRelationID="", $bwlIsPK=false)
{
	...
	$out.="<td colspan=\"9\">help text<br><textarea id=\"id" . qpre . "helptext-" .  $outcount . "\" name=\"" . qpre . "helptext-" .  $outcount . "\" rows=\"3\" cols=\"42\">" .  $helptext . "</textarea>";
	$out.="<br/>extra functionality<br><textarea id=\"id" . qpre . "extra_functionality-" .  $outcount . "\" name=\"" . qpre . "extra_functionality-" .  $outcount . "\" rows=\"3\" cols=\"42\">" .  $extra_functionality . "</textarea>";
	$out.="<br/>";
	...
	$out.="<br/>";
	$out.="Force Editable " .   boolcheck(qpre . "forceeditable-" . $outcount , $forceeditable, false, false, $strStyle);
	$out.="<br/>";
	$out.="Force Uneditable " .   boolcheck(qpre . "forceuneditable-" . $outcount , $forceuneditable, false, false, $strStyle);
	$out.="<br/>";
...
function AlterTable($strDatabase, $strTable,  $strOldTableName,  $strTypeConfig)
{
	...
	for($i=0; $i<=$count; $i++)
	{
		...
		$extra_functionality=$_REQUEST[qpre . "extra_functionality-" . $i];
		...
		$forceeditable=$_REQUEST[qpre . "forceeditable-" . $i];
		$forceuneditable=$_REQUEST[qpre . "forceuneditable-" . $i];
		...
		$whereclause=" table_name='" . AppropriateSQLName($strTable) . "' AND column_name='" .  removeTics(ensureEnclosure(AppropriateSQLName($thisField)), "`", "`")  . "'";
		//this code may need to be changed!!!!
		if($extra_functionality!="" || $helptext!=""  || $friendly!="" || $validation_type_id!=""  || $invisible!="" || $force_no_wysiwg!="" || $force_wysiwg!=""  || $force_no_bianry!="" || $force_binary!="" || $fileupload!="" || $datecreated!="" || $datemodified!=""  || $forceeditable!=""  || $forceuneditable!="" || $password!="" || $validation_pattern_id!="" || $width!=""  || $height!="")
		{
			...
			$strPostExtraSQL= $strDatabase . "." .  tfpre . "column_info SET ";
  			$strPostExtraSQLValues=IfAThenB($helptext,",help_text='" .  ourDBescape($helptext) . "'") . 
			IfAThenB($extra_functionality,",extra_functionality='" .  ourDBescape($extra_functionality) . "'") . 
			...
			IfAThenB($forceeditable,",forceeditable='" . $forceeditable . "'") .
			IfAThenB($forceuneditable,",forceuneditable='" . $forceuneditable . "'") .
		...
		if ($thisField!="")
		{
			...
			if ($oldField!="")
			{
				...
				if ($thisField!="")
				{
					...
					if ($possiblelocation=="*")
					{
						...
						{
							$strSQL="ALTER TABLE " . $strDatabase . "." .$strTable . "   CHANGE " . ensureEnclosure($oldField, "`", "`") . " " . ensureEnclosure(AppropriateSQLName($thisField), "`", "`") . " " . TypeSQL($thisType, $thisLength) . $strSQLDesc;
					...
					elseif ($possiblelocation!="")
					{
						if ($oldField !=$thisField)
						{
							$strSQL="ALTER TABLE " . $strDatabase . "." .$strTable . "   CHANGE " . ensureEnclosure(AppropriateSQLName($oldField), "`", "`")  . " " .  ensureEnclosure(AppropriateSQLName($thisField), "`", "`")   . " " . TypeSQL($thisType, $thisLength) . $strSQLDesc ;
			...
			else
			{
				if ($strPrevious!="") 
				{
					$strSQL="ALTER TABLE " . $strDatabase  . "." . $strTable . "  ADD  " . ensureEnclosure(AppropriateSQLName($thisField), "`", "`") .  " "   . TypeSQL($thisType, $thisLength)  . $strSQLDesc  ." AFTER " .  ensureEnclosure($strPrevious, "`", "`") ;
				}
				else
				{
					$strSQL="ALTER TABLE " . $strDatabase  . "." . $strTable . "  ADD "  . ensureEnclosure(AppropriateSQLName($thisField), "`", "`") . " " . TypeSQL($thisType, $thisLength) . $strSQLDesc;
				}
		...
		if ($strSQL!="")
		{
			...
			$errors.=IfAThenB(sql_error(), "<b>failed to alter table (1):</b>" .  sql_error() . "<br>". $strSQL . "<br><br>");
			//echo sql_error() . "<br>";
	...
	if ($newtable)
	{
		...
		$failedtoaltererror="";
		$failedtoaltererror=IfAThenB(sql_error(), "<b>failed to alter table (2):</b>" .  sql_error() . "<br>". $strSQL . "<br><br>");
...
function deletefields($strDatabase, $strTable, $deletefields)
{
	...
	for ($i=0; $i<count($arrThis); $i++)
	{
		...
		if ($strThisField!="")
		{
			...
			$strSQL="ALTER TABLE  " . $strDatabase  . "." . $strTable . "  DROP COLUMN " . ensureEnclosure(AppropriateSQLName($strThisField), "`", "`");
...
//moves a column in the database
{
	$sql=conDB();
 	...
 	if($after!="")
	{
		$strSQL="ALTER TABLE " . $strDatabase . "." .$strTable . "   MODIFY COLUMN " . ensureEnclosure(AppropriateSQLName($this), "`", "`") . "  " . TypeSQL($strType, $strSize) . " AFTER " . $after;
	}
	else
 	{
		$strSQL="ALTER TABLE " . $strDatabase . "." .$strTable . "   MODIFY COLUMN " . ensureEnclosure(AppropriateSQLName($this), "`", "`") . " " .  TypeSQL($strType, $strSize) ." FIRST";
	}
	//echo $strSQL . "<br>";
	if(CanChange())
	{
